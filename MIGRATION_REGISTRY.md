...
> آخر تحديث: 2026-08-13 — دمج 0375→0408 (34 migration: authz، idempotency، rate limits، weekly summary، mobile ops).

## الإحصائيات الفعلية الحالية

| العنصر | العدد |
|---|---|
| إجمالي الملفات `.sql` المرقمة | 405: 0001–0408 (يشمل placeholders الموثقة) |
| تكرارات نشطة | ✅ لا شيء |
| فجوات | ✅ لا شيء (جسور موثقة: 0119, 0122, 0194, 0219, 0231, 0232, 0270-0276, 0297-0300, 0302, 0304, 0313, 0322, 0332, 0377 — والفجوات 0267/0279/0314/0315/0338/0395 مقبولة وموثّقة) |
...
| 0459 | `0459_fix_manager_final_column.sql` | advance_kpi_stage (من 0457) كانت تستخدم manager_final_comment غير الموجود — الاستبدال بـ manager_comment الموحّد للمرحلتين. |
...
| 0461 | `0461_punch_reminder_advisory_lock.sql` | قفل استشاري (advisory lock) لـ generate_punch_reminders يمنع تداخل تشغيلي مزدوج لو تأخر cron كل 5 دقائق وتلاقى تشغيلان — يمنع تكرار تنبيهات «نسيت البصمة». |
...
| 0463 | `0463_cairo_today_canonical.sql` | توحيد «اليوم» على توقيت القاهرة (`(now() at time zone 'Africa/Cairo')::date`) بدل current_date المرتبط بمنطقة الجلسة في الدوال التنفيذية (attendance_today/overview/fast + طلب تصحيح الحضور) — نفس جذر انزياح 0458/0460. |
| 0464 | `0464_hr_full_access_and_inbox.sql` | HR صلاحية كاملة غير مقيدة (أي طلب/مرحلة/وقت/يوم حتى بعد التصعيد، وحتى طلبه الشخصي بإذن صريح) + المدير المباشر يعتمد أي وقت حتى بعد تصعيد الطلب لأبو عمار + أبو عمار للمتصعَّد (0440) + صندوق الإجراءات يرى كل فاعل ما يخصه (إصلاح الفارغ مع إعادة شرط 0427 الشامل). |
| 0471 | `0471_hide_no_balance_leave_boxes.sql` | إخفاء صناديق الأرصدة بلا معنى: الإجازة المرضية (بلا حد، تُطلب مباشرة) والإجازة بدون أجر (بلا رصيد) من get_my_leave_balances — تبقى annual/casual/weekly_rest_comp. استثناء بالكود لا بالأعلام لأن بدل الراحة affects_balance=false لكن له رصيد حقيقي. |
| 0465 | `0465_fix_sla_escalation_flood.sql` | إيقاف فيض التصعيد: مهلة الخطوة 1 تمتد +24 ساعة قبل التصعيد للخطوة 2، ومنع تكرار صفوف escalated النشطة لنفس الطلب، وتخفيف تنبيهات SLA المتكررة (فقط عند تجاوز 7 أيام يرفع خطورة high). |
| 0466 | `0466_notification_retention.sql` | سياسة احتفاظ لسجل الإشعارات: purge_old_failed_notification_jobs يحذف مهام push الفاشلة الأقدم من 30 يومًا (حد أدنى حفظ 7 أيام، دفعات آمنة) + جدولة يومية 03:45 باسم hr_notification_retention + تسجيل التشغيل في cron_health_log — service_role فقط. |
| 0467 | `0467_cron_health_queue_lag.sql` | توسيع verify_critical_cron_jobs برصد «موجودة لكن معطّلة»: تنبيه P1 إن توقف نجاح ساحب الإشعارات >10 دقائق أو تأخر طابور التكاملات >15 دقيقة، وP2 عند تراكم 50+ إشعار push فاشل خلال أسبوع — بنمط فتح/حسم system_alerts ذاته. |
| 0473 | `0473_fix_push_spike_alert_severity.sql` | إصلاح عاجل لـ0467: خطورة تنبيه الفيض كانت 'P2' يرفضها قيد system_alerts_severity_check (يسمح P0/P1 فقط) فانهارت الدالة عند أول فيض حقيقي (رُصد في CI) — استبدال قياسي كامل بخطورة P1 ومصادر أدق (queue/notification). مُتحقق بمحاكاة فيض كاملة داخل معاملة ملغاة على الإنتاج. |
| 0475 | `0475_fix_queue_lag_null_guard.sql` | حرس NULL لتنبيهَي تأخر الطوابير من 0467: غياب سجل النجاح أو غياب صفوف pending على قاعدة حديثة كان يفسَّر '-infinity' فيفتحا زوراً بعد كل db reset (CI: have:2 want:0) — الشرطان الآن يتطلبان دليلاً تشغيلياً فعلياً قبل الإنذار. مُتحقق بمحاكاة حذف job_run_details كاملاً داخل معاملة ملغاة. |
| 0540 | `0540_hotfix_disputes_and_security.sql` | إصلاحات حرجة: عمود cancelled_reason على instant_attendance_penalties، استبدال user_has_permission غير الموجودة باستعلام مباشر، سحب anon من دوال تنفيذية، تريجر updated_at لـ penalty_disputes، أكواد خطأ في RAISE EXCEPTION، تسجيل أخطاء bulk operations. |
| 0541 | `0541_fix_read_function_volatility.sql` | تصحيح فولاتيليتي الدوال القارئة (VOLATILE → STABLE) لتحسين الأداء وخطة التنفيذ. |
| 0542 | `0542_fix_instant_penalties_anon_leak_and_pagination.sql` | إغلاق تسريب get_instant_penalties لغير المصادَقين (حارس مقلوب كان يسمح لـ anon بقراءة كل الغرامات) + إصلاح الترقيم limit/offset داخل استعلام فرعي + سحب anon من دالتي الإعفاء. |
| 0543 | `0543_revoke_anon_from_inverted_guard_functions.sql` | تحصين 11 دالة ذات «الحارس المقلوب» بسحب anon/public صراحةً — منع تكرار انحدار 0536/0541. |
| 0544 | `0544_attendance_corrections_team_review.sql` | دعم شاشة تفاصيل تصحيح البصمة (get_attendance_correction_detail) + قائمة تصحيحات الفريق (get_team_attendance_corrections) + توجيه إشعارات التصحيح attendance_correction_detail. |
| 0545 | `0545_fix_mission_approval_check_constraint.sql` | إصلاح خطأ 23514 (ck_missions_period) عند اعتماد المأموريات في tg_leave_attendance_on_approval بحساب end_at كـ T23:59:59 افتراضياً وضمان end_at >= start_at. |
| 0548 | `0548_block_self_exemption_and_name_based_immunity.sql` | منع الإعفاء الذاتي من الحضور/الغرامات بحارس أعمدة جديد (tg_employees_protect_exemption_identity) + إزالة مطابقة الإعفاء بالاسم (ilike) التي كانت تمنح حصانة لأي موظف يطابق الاسم + سحب anon (انحدار 0547). |
| 0549 | `0549_support_all_mobile_action_targets.sql` | دعم instant_penalty/daily_report/device/fellowship_fund في get_mobile_action_target + استبدال رمي 22023 بكائن mobileRoute='unsupported' آمن يمنع تجميد الموبايل. (كان 0547 — أُعيد ترقيمه لتصادمه.) |
| 0550 | `0550_escalation_doubling_auto_suspension_manual.sql` | المضاعفة تلقائية والتعليق قرار بشري: v_can_suspend يطابق شرطَي حارسي employees/profiles حرفياً؛ إن لم يتحقق تُسجَّل suspension_pending وتبقى المضاعفة محفوظة (كان الكرون يلغي الدورة كاملة عند أول صف للتعليق). |
| 0551 | `0551_revoke_public_execute_from_secdef.sql` | سحب EXECUTE من PUBLIC/anon على كل دوال SECURITY DEFINER (المنحة الافتراضية PUBLIC=X كانت تجعل has_function_privilege(anon)=true حتى بعد سحب anon الصريح) — بنمط 0209 مع استثناءات get_public_* المقصودة + ALTER DEFAULT PRIVILEGES لمنع الانحدار. |
...
>
> **سكربت النشر المعمّم لأي نطاق مستقبلي:** `python scripts/deploy_migrations.py --from 0456 [--to NNNN] [--force]` — نفس الضمانات مع أولوية للنطاق الحر.
---
...
> ✅ **الحالة:** سلسلة متصلة — 0001 → 0408 (405 ملف) — بلا تكرار أو فجوات. (تم التحقّق عبر `node scripts/check-migrations-integrity.mjs` في 2026-08-13.)
