...
function AuthenticatedApp() {
  ...
  if (auth.access.isSuspended) {
    return (
      <main className="grid min-h-screen place-items-center p-6 bg-slate-900 text-white">
        <section className="card max-w-lg p-8 text-center bg-slate-800 border-red-500/30 rounded-2xl shadow-2xl">
          ...
          <h1 className="text-2xl font-black text-red-400">تم إيقاف الحساب عن العمل</h1>
          <p className="mt-4 text-base font-medium leading-relaxed text-slate-200">
            {auth.access.suspensionMessage || 'تم وقفك عن العمل لعدم سداد قيمة الخصم 500جنيه توجه الي قسم ال HR لسداد المبلغ'}
          </p>
          <div className="mt-6 flex flex-col gap-2">
            <button
              className="w-full rounded-xl bg-slate-700 hover:bg-slate-600 px-4 py-2.5 font-bold text-white transition"
              onClick={() => void auth.refreshAccess()}
            >
              إعادة التحقق من حالة الحساب
            </button>
            <button
              className="w-full rounded-xl bg-red-600/80 hover:bg-red-600 px-4 py-2.5 font-bold text-white transition"
