  ...
  return <div className="space-y-6">
    ...
    <section className="grid gap-6 xl:grid-cols-[380px_1fr]">
      <form className="card space-y-3 p-5" onSubmit={(e) => { e.preventDefault(); void saveShift(); }}>
        ...
        <button className="btn-primary w-full" disabled={commands.saveShift.isPending}><Save className="size-4" aria-hidden="true" />حفظ الوردية</button>
      </form>
      <section className="card p-5"><h2 className="text-lg font-black">الورديات الحالية</h2><div className="mt-4 grid gap-3 sm:grid-cols-2">{data?.shifts.map((item) => <button key={item.id} className="card-interactive rounded-2xl border border-[var(--border)] p-4 text-start" onClick={() => setShift({ id: item.id, name: item.name, start: item.startTime.slice(0, 5), end: item.endTime.slice(0, 5), breakMinutes: item.breakMinutes, graceIn: item.graceInMinutes, graceOut: 0, active: item.active })}><div className="flex items-center justify-between"><strong>{item.name}</strong><StatusBadge value={item.active ? 'active' : 'inactive'} /></div><p className="muted mt-2 text-sm">{item.startTime.slice(0, 5)} — {item.endTime.slice(0, 5)} · سماح {item.graceInMinutes} دقيقة</p></button>)}</div></section>
    </section>
