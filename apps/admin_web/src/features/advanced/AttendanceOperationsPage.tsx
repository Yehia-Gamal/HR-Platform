import { CalendarClock, CheckCircle2, Clock3, RotateCcw, Save, UsersRound } from 'lucide-react';
...
export function AttendanceOperationsPage() {
  ...
  return <div className="space-y-6">
    <PageHeader title="الورديات وإغلاق الحضور" description="إدارة تعريفات الورديات، جداول العمل، تصحيحات الحضور، العمل الإضافي، وإغلاق الشهر بسجل تدقيق." actions={<label className="text-sm font-bold"><span className="block">الشهر</span><input type="month" aria-label="الشهر" className="input mt-1" value={month} onChange={(e) => setMonth(e.target.value)} /></label>} />
    ...
    <section className="card p-5"><div className="flex flex-wrap items-center justify-between gap-3"><h2 className="text-lg font-black">تصحيحات الحضور</h2><span className="muted text-sm">الموافقة تعدّل سجل اليوم خادميًا، والرفض يحتاج سببًا.</span></div>
      {data?.corrections.length === 0 ? <EmptyState title="لا توجد تصحيحات" description="لا توجد طلبات تصحيح في الشهر المحدد." /> : <div className="mt-4 space-y-3">{data?.corrections.map((item) => <article key={item.id} className="rounded-2xl border border-[var(--border)] p-4"><div className="flex flex-wrap items-center justify-between gap-2"><div><strong>{item.employeeName}</strong><p className="muted text-sm">{item.workDate} · {item.type} · {item.reason}</p></div><StatusBadge value={item.status} /></div>{item.status === 'pending' ? <div className="mt-3 grid gap-2 md:grid-cols-[1fr_auto_auto]"><input className="input" placeholder="ملاحظة القرار" value={reviewNotes[item.id] ?? ''} onChange={(e) => setReviewNotes((v) => ({ ...v, [item.id]: e.target.value }))} /><button className="btn-primary" onClick={() => void commands.decideCorrection.mutateAsync({ p_id: item.id, p_decision: 'approved', p_note: reviewNotes[item.id] || null })}>اعتماد</button><button className="btn-secondary" onClick={() => void commands.decideCorrection.mutateAsync({ p_id: item.id, p_decision: 'rejected', p_note: reviewNotes[item.id] || '' })}>رفض</button></div> : null}</article>)}</div>}
    </section>
