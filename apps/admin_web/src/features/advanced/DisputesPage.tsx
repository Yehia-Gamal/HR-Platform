    ...
    <section className="grid gap-6 xl:grid-cols-[390px_minmax(0,1fr)]">
      <div className="card p-4">
        ...
        <div className="mt-4 max-h-[calc(100vh-280px)] space-y-2 overflow-y-auto ps-1">
          {query.isLoading ? <ListSkeleton rows={4} label="جارٍ تحميل القضايا" /> : filteredCases.map((item) => <button key={item.id} type="button" onClick={() => chooseCase(item)} className={`w-full rounded-2xl border p-4 text-start transition ${selectedCase === item.id ? 'border-[var(--brand-primary)] bg-[var(--surface-muted)]' : 'border-[var(--border)] hover:border-[var(--border-strong)]'}`}>
      ...
      {selected ? <div className="min-w-0 space-y-5">
        ...
        {selected.members.length >= 2 ? <section className="card p-5">
          ...
          <div className="mt-5 space-y-2">{selected.sessions.map((item) => <button key={item.id} type="button" className="w-full rounded-xl border border-[var(--border)] p-3 text-start" onClick={() => { setMinutes({ ...minutes, sessionId: item.id }); setDecision({ ...decision, sessionId: item.id }); setAttendance(Object.fromEntries(selected.members.filter((member) => member.active).map((member) => [member.id, 'present']))); }}><div className="flex flex-wrap items-center justify-between gap-2"><span>{item.type} · {formatDate(item.scheduledAt)} · {item.location ?? 'دون مكان'}</span><StatusBadge value={item.status} /></div>{item.minutes ? <p className="muted mt-2 line-clamp-2 text-xs">{item.minutes}</p> : null}</button>)}</div>
        </section> : null}
