          ...
          {selected.sessions
            ...
            .map((item) => (
              <option value={item.id} key={item.id}>
                {item.type} — {formatDate(item.heldAt, false)}
              </option>
            ))}
        </select>
        <textarea
        ...
        <div className="card p-4">
          ...
          <div className="mt-4 max-h-[calc(100vh-280px)] space-y-2 overflow-y-auto ps-1">
            {query.isLoading ? (
              ...
              filteredCases.map((item) => <CaseListItem key={item.id} item={item} isSelected={selectedCase === item.id} onSelect={() => chooseCase(item)} />)
            )}
          </div>
        ...
        {selected ? (
          <div key={selected.id} className="min-w-0 space-y-5">
            ...
            {['submitted', 'needs_more_information'].includes(selected.status) ? (
              <AcceptCaseForm selected={selected} people={people} commands={commands} run={run} />
            ) : null}
            ...
            {[
              ...
              <CommitteeForm selected={selected} people={people} commands={commands} run={run} />
            ) : null}
            ...
            {selected.sessions.some((item) => item.status === 'held') && !selected.decision ? (
              <DecisionForm selected={selected} people={people} commands={commands} run={run} />
            ) : null}
