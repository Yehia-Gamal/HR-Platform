...
type RunFn = (task: () => Promise<unknown>, success: string) => Promise<void>;
...
function CaseListItem({ item, isSelected, onSelect }: { item: DisputeCase; isSelected: boolean; onSelect: () => void }) {
  return (
    <button type="button" onClick={onSelect} className={`w-full rounded-2xl border p-4 text-start transition ${isSelected ? 'border-[var(--brand-primary)] bg-[var(--surface-muted)]' : 'border-[var(--border)] hover:border-[var(--border-strong)]'}`}>
...
function CaseDetailHeader({ selected }: { selected: DisputeCase }) {
  return (
...
function AcceptCaseForm({ selected, people, commands, run }: { selected: DisputeCase; people: Person[]; commands: Commands; run: RunFn }) {
  ...
  return (
    <section className="card p-5">
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-[1fr_140px_auto]">
        ...
        <button className="btn-primary" disabled={!assignedTo || commands.acceptCase.isPending} onClick={() => void run(() => commands.acceptCase.mutateAsync({ p_case_id: selected.id, p_assigned_to: assignedTo, p_quorum: quorum, p_due_at: null }), 'تم قبول المشكلة وبدأ المسار الرسمي.')}><CheckCircle2 className="size-4" aria-hidden="true" />قبول المشكلة</button>
      </div>
    </section>
  );
}

function TransitionForm({ selected, commands, run }: { selected: DisputeCase; commands: Commands; run: RunFn }) {
  ...
  return (
    <section className="card p-5">
      ...
      <button className="btn-primary mt-3" disabled={!transition.action || commands.transitionCase.isPending || (['request_respondent_statement', 'request_witness_statement'].includes(transition.action) && !transition.targetId)} onClick={() => void submitTransition()}><Send className="size-4" aria-hidden="true" />تنفيذ وتسجيل الإجراء</button>
    </section>
  );
}

function CommitteeForm({ selected, people, commands, run }: { selected: DisputeCase; people: Person[]; commands: Commands; run: RunFn }) {
  const [members, setMembers] = useState(selected.members.filter((member) => member.active).map((member) => ({ employeeId: member.employeeId, role: member.role })));

  return (
    <section className="card p-5">
      <div className="flex items-center justify-between"><div><h3 className="text-lg font-black">تشكيل اللجنة</h3><p className="muted mt-1 text-sm">لا يجوز اختيار أي طرف في المشكلة عضوًا باللجنة.</p></div><button className="btn-secondary" onClick={() => setMembers((current) => [...current, { employeeId: '', role: current.length ? 'member' : 'chair' }])}><UsersRound className="size-4" aria-hidden="true" />إضافة عضو</button></div>
      ...
      <button className="btn-primary mt-4" disabled={members.filter((item) => item.employeeId).length < 2 || !members.some((item) => item.role === 'chair') || commands.setCommittee.isPending} onClick={() => void run(() => commands.setCommittee.mutateAsync({ p_case_id: selected.id, p_members: members.filter((item) => item.employeeId) }), 'تم حفظ تشكيل اللجنة وبدء المراجعة.')}><UsersRound className="size-4" aria-hidden="true" />حفظ تشكيل اللجنة</button>
    </section>
  );
}

function NotesSection({ selected, commands, run }: { selected: DisputeCase; commands: Commands; run: RunFn }) {
...
function SessionsSection({ selected, commands, run }: { selected: DisputeCase; commands: Commands; run: RunFn }) {
  ...
  return (
    <section className="card p-5">
      ...
      <div className="mt-4 grid gap-3 lg:grid-cols-3">
        ...
        <button className="btn-primary" disabled={!session.at || commands.scheduleSession.isPending} onClick={() => void run(() => commands.scheduleSession.mutateAsync({ p_case_id: selected.id, p_type: session.type, p_scheduled_at: new Date(session.at).toISOString(), p_ends_at: session.endsAt ? new Date(session.endsAt).toISOString() : null, p_location: session.location || null, p_modality: session.modality, p_participants: [...selected.parties.map((party) => ({ employeeId: party.employeeId, role: party.type })), ...selected.members.map((member) => ({ employeeId: member.employeeId, role: 'committee' }))] }), 'تم تحديد الجلسة وإشعار المشاركين دون كشف تفاصيل سرية.')}><CalendarPlus className="size-4" aria-hidden="true" />تحديد جلسة</button>
      </div>
      <div className="mt-5 space-y-2">{selected.sessions.map((item) => <div key={item.id} className="w-full rounded-xl border border-[var(--border)] p-3 text-start"><div className="flex flex-wrap items-center justify-between gap-2"><span>{item.type} · {formatDate(item.scheduledAt)} · {item.location ?? 'دون مكان'}</span><StatusBadge value={item.status} /></div>{item.minutes ? <p className="muted mt-2 line-clamp-2 text-xs">{item.minutes}</p> : null}</div>)}</div>
    </section>
  );
}

function MinutesForm({ selected, commands, run }: { selected: DisputeCase; commands: Commands; run: RunFn }) {
  const [minutes, setMinutes] = useState({ sessionId: '', text: '', outcome: '', recommendation: '', internalNotes: '' });
  const [attendance, setAttendance] = useState<Record<string, string>>(Object.fromEntries(selected.members.filter((member) => member.active).map((member) => [member.id, 'present'])));
  ...
  return (
    <section className="card p-5">
      ...
      <div className="mt-4 grid gap-3"><select className="input" aria-label="اختر جلسة مكتملة" value={decision.sessionId} onChange={(event) => setDecision({ ...decision, sessionId: event.target.value })}><option value="">اختر جلسة مكتملة</option>{selected.sessions.filter((item) => item.status === 'held').map((item) => <option value={item.id} key={item.id}>{item.type} — {formatDate(item.heldAt, false)}</option>)}</select>
        ...
        <button className="btn-primary" disabled={!decision.sessionId || decision.text.trim().length < 20 || decision.rationale.trim().length < 20 || commands.issueDecision.isPending} onClick={() => void run(() => commands.issueDecision.mutateAsync({ p_case_id: selected.id, p_session_id: decision.sessionId, p_text: decision.text, p_rationale: decision.rationale, p_outcome: decision.outcome, p_owner_id: decision.ownerId || null, p_due_at: decision.dueAt ? new Date(decision.dueAt).toISOString() : null }), 'صدر القرار وأُبلغ الأطراف ومسؤول التنفيذ.')}><Gavel className="size-4" aria-hidden="true" />إصدار القرار</button>
      </div>
    </section>
  );
}
...
function DecisionDisplay({ selected }: { selected: DisputeCase }) {
  ...
  return (
    <section className="card p-5"><div className="flex flex-wrap items-start justify-between gap-3"><div><h3 className="text-lg font-black">القرار {d.number}</h3><p className="muted mt-1 text-sm">صدر {formatDate(d.issuedAt)}</p></div><StatusBadge value={d.status} /></div><p className="mt-4 whitespace-pre-wrap leading-8">{d.text}</p><details className="mt-4 rounded-2xl bg-[var(--surface-muted)] p-4"><summary className="cursor-pointer font-black">الحيثيات الداخلية</summary><p className="mt-3 whitespace-pre-wrap leading-7">{d.rationale}</p></details></section>
  );
}

function AdminActionPipeline({ selected, commands, run }: { selected: DisputeCase; commands: Commands; run: RunFn }) {
    ...
    <section className="card p-5">
      ...
      {!selected.proposedAdminAction ? <div className="mt-5 rounded-2xl border border-[var(--border)] p-4">
        ...
        <div className="mt-3 grid gap-3 sm:grid-cols-[200px_1fr_auto]">
          ...
          <button className="btn-primary self-end" disabled={!proposedAction || proposedActionDetail.trim().length < 3 || commands.proposeAdminAction.isPending} onClick={() => void run(async () => { await commands.proposeAdminAction.mutateAsync({ p_case_id: selected.id, p_proposed_action: proposedAction, p_detail: proposedActionDetail.trim() }); setProposedAction(''); setProposedActionDetail(''); }, 'تم إرسال الاقتراح للمدير التنفيذي للمراجعة.')}><Briefcase className="size-4" aria-hidden="true" />إرسال الاقتراح</button>
        </div>
      </div> : null}
      ...
      {selected.executiveDecision === 'modified' && !selected.executedAt ? <div className="mt-5 rounded-2xl border border-[var(--warning)]/30 bg-[var(--warning-soft)] p-4">
        ...
        <div className="mt-3 flex flex-col gap-3 sm:flex-row">
          <textarea className="input min-h-20 flex-1" placeholder="ملاحظات التنفيذ وتفاصيل ما تم…" value={executionNotes} onChange={(event) => setExecutionNotes(event.target.value)} aria-label="ملاحظات التنفيذ" />
          <button className="btn-primary self-end" disabled={executionNotes.trim().length < 5 || commands.executeAdminAction.isPending} onClick={() => void run(async () => { await commands.executeAdminAction.mutateAsync({ p_case_id: selected.id, p_notes: executionNotes.trim() }); setExecutionNotes(''); }, 'تم تنفيذ الإجراء الإداري المعدّل وتسجيله.')}><ClipboardCheck className="size-4" aria-hidden="true" />تأكيد التنفيذ</button>
        </div>
      </div> : null}
...
export function DisputesPage() {
  ...
  return <div className="space-y-6">
    ...
    <section className="grid gap-6 xl:grid-cols-[390px_minmax(0,1fr)]">
      <div className="card p-4">
        ...
        <div className="mt-4 max-h-[calc(100vh-280px)] space-y-2 overflow-y-auto ps-1">
          {query.isLoading ? <ListSkeleton rows={4} label="جارٍ تحميل القضايا" /> : filteredCases.map((item) => <CaseListItem key={item.id} item={item} isSelected={selectedCase === item.id} onSelect={() => chooseCase(item)} />)}
        </div>
      ...
      {selected ? <div key={selected.id} className="min-w-0 space-y-5">
        ...
        <CaseDetailHeader selected={selected} />

        {['submitted', 'needs_more_information'].includes(selected.status) ? <AcceptCaseForm selected={selected} people={people} commands={commands} run={run} /> : null}

        <TransitionForm selected={selected} commands={commands} run={run} />

        {['accepted', 'under_review', 'waiting_for_respondent', 'waiting_for_witness', 'returned_to_committee', 'reopened', 'session_scheduled', 'committee_deliberation'].includes(selected.status) ? <CommitteeForm selected={selected} people={people} commands={commands} run={run} /> : null}

        {selected.members.length ? <NotesSection selected={selected} commands={commands} run={run} /> : null}

        {selected.members.length >= 2 ? <SessionsSection selected={selected} commands={commands} run={run} /> : null}

        {selected.sessions.some((item) => item.status === 'scheduled') ? <MinutesForm selected={selected} commands={commands} run={run} /> : null}

        {selected.sessions.some((item) => item.status === 'held') && !selected.decision ? <DecisionForm selected={selected} people={people} commands={commands} run={run} /> : null}

        {selected.decision ? <DecisionDisplay selected={selected} /> : null}

        {selected.decision ? <AdminActionPipeline selected={selected} commands={commands} run={run} /> : null}

        {selected.decision && selected.status !== 'closed' ? <SettlementForm selected={selected} commands={commands} run={run} /> : null}

        {selected.actions.length ? <ActionTracker selected={selected} commands={commands} run={run} /> : null}

        {selected.appeals.length ? <AppealsPanel selected={selected} commands={commands} run={run} /> : null}

        <PartiesPanel selected={selected} />
    ...
    {(summary?.critical ?? 0) > 0 ? <div role="alert" className="fixed bottom-5 start-5 z-30 flex max-w-sm items-start gap-3 rounded-2xl border border-[var(--danger)]/30 bg-[var(--surface)] p-4 shadow-xl"><AlertTriangle className="mt-0.5 size-5 shrink-0 text-[var(--danger)]" aria-hidden="true" /><div><strong>توجد قضايا حرجة</strong><p className="muted mt-1 text-xs">راجعها فورًا وحدد ما إذا كانت تحتاج تصعيدًا للمدير التنفيذي.</p></div></div> : null}
  </div>;
}
