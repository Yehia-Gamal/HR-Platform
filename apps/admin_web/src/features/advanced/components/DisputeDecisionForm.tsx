...
import type { DisputeCase, Person, Commands, RunFn } from './DisputeTypes';
          ...
          {selected.sessions
            ...
            .map((item) => (
              <option value={item.id} key={item.id}>
                {item.type} — {item.heldAt ? new Intl.DateTimeFormat('ar-EG', { dateStyle: 'medium' }).format(new Date(item.heldAt as string)) : '—'}
              </option>
            ))}
        </select>
        <textarea
