...
export function useOrganizationLookups() {
  const auth = useAuth();
  return useQuery({
    queryKey: ['organization-lookups', auth.isMock],
    enabled: auth.status === 'authenticated',
    staleTime: 5 * 60_000,
    queryFn: async (): Promise<OrganizationLookups> => {
      ...
      return {
        ...
        roles: rows(data.roles)
          .filter((r) => !r.is_full_access && !r.is_capability)
          .map((r) => ({ id: String(r.id), slug: String(r.slug), label: String(r.name_ar) })),
        // إخفاء كود الموظف إذا كان رقم هاتف (يبدأ بـ + أو 01) — المشتق تلقائياً من الهاتف
        managers: rows(data.employees).map((r) => ({
          id: String(r.id),
          label: /^\+|^01\d/.test(String(r.employee_code)) ? String(r.full_name_ar) : `${String(r.full_name_ar)} · ${String(r.employee_code)}`,
        })),
