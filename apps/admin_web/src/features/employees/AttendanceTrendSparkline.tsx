...
export function AttendanceTrendSparkline({ points }: { points: AttendanceTrendPoint[] }) {
  ...
  return (
    <div className="mt-2 flex flex-col gap-1 border-t border-[var(--border)] pt-2">
      ...
      <svg role="img" aria-label="اتجاه نسبة الحضور آخر 7 أيام" width={width} height={height} viewBox={`0 0 ${width} ${height}`} className="overflow-visible">
        {data.map((p, i) => {
          ...
          return (
            <rect key={p.date} x={x} y={y} width={Math.max(1, barWidth)} height={barH} rx={1.5} fill={fill} opacity={0.9}>
