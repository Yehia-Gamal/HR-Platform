...
export function EmployeeTasksTab({ employeeId }: { employeeId: string }) {
  ...
  if (query.isError) {
    return <ErrorState title="تعذر تحميل المهام" description="أعد المحاولة أو تواصل مع الدعم." onRetry={() => void query.refetch()} />;
  }
            ...
            {rows.map((row) => (
              <tr key={row.id} className="border-b border-[var(--border-subtle)] last:border-0">
                <td className="px-4 py-3">
                  ...
                  {row.description ? <p className="muted mt-0.5 text-xs">{row.description}</p> : null}
                </td>
                <td className="px-4 py-3">
                  <StatusBadge value={row.priority} />
                </td>
                <td className="px-4 py-3">{row.dueDate ? dateFormatter.format(new Date(`${row.dueDate}T00:00:00`)) : '—'}</td>
                <td className="px-4 py-3">
