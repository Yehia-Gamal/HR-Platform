...
export function KpiEvaluationEditor({ evaluationId, onDone }: { evaluationId: string; onDone: () => void }) {
  ...
  return <section className="card overflow-hidden">
    <header className="flex flex-wrap items-start justify-between gap-4 border-b border-[var(--border)] p-5">
      ...
      <div className="text-end"><p className="text-2xl font-black">{form.finalScore ?? form.criteria.reduce((sum, item) => sum + (item.effectiveScore ?? 0), 0)} / 100</p><p className="muted text-xs">الموعد: {form.cycle.effectiveDeadline ? new Date(form.cycle.effectiveDeadline).toLocaleString('ar-EG') : '—'}</p>{form.parallelFlow ? <p className="mt-1 text-xs font-bold text-brand">مسار V23 المتوازي</p> : null}<button className="btn-secondary mt-2" onClick={onDone}>إغلاق التفاصيل</button></div>
    </header>
