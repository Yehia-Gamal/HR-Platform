...
// V23: أضفنا المراحل الجديدة (المراجعة المتوازية، السكرتير، المدير التنفيذي).
const stageLabel: Record<string, string> = {
  self: 'التقييم الذاتي',
  parallel_review: 'مراجعة متوازية (HR + المدير)',
  manager_review: 'مراجعة المدير المباشر',
  hr_review: 'مراجعة HR',
  manager_final: 'اعتماد المدير النهائي',
  secretary_review: 'مراجعة السكرتير التنفيذي',
...
export function KpiEvaluationEditor({ evaluationId, onDone }: { evaluationId: string; onDone: () => void }) {
  ...
  // V23: في المراجعة المتوازية، HR والمدير يعملان معًا — نُظهر تبويبات الامتثال والجلسة.
  const isParallel = form?.editableStage === 'parallel_review';
  const showCompliance = form?.editableStage === 'hr_review' || isParallel;
  const showSession = form?.editableStage === 'manager_review' || isParallel;
  // V23: مراحل تتطلب إرسال درجات عند الاعتماد.
  const scoreSendStages = ['self', 'manager_review', 'parallel_review'];
  ...
  const submit = async () => {
    ...
    await advance.mutateAsync({
      ...
      action: form.editableStage,
      note,
      scores: scoreSendStages.includes(form.editableStage) ? editableCriteria.map((criterion) => ({ criterion_id: criterion.id, score: scores[criterion.id]?.score ?? 0, note: scores[criterion.id]?.note ?? '' })) : undefined,
    });
    onDone();
  };
  ...
  const submitLabel = (() => {
    switch (form.editableStage) {
      case 'manager_review': return 'اعتماد النتيجة وإدراجها في التقرير';
      case 'parallel_review': return 'اعتماد مراجعتي وإرسال';
      case 'secretary_review': return 'اعتماد السكرتير وإرسال للمدير التنفيذي';
      case 'executive_review': return 'إقرار المدير التنفيذي والاعتماد النهائي';
      default: return 'اعتماد المرحلة وإرسال';
    }
  })();

  // V23: أهداف الإرجاع حسب المرحلة.
  const returnTarget = (() => {
    switch (form.editableStage) {
      case 'hr_review': return 'self';
      case 'manager_review': return 'hr_review';
      case 'parallel_review': return 'self';
      case 'secretary_review': return 'parallel_review';
      case 'executive_review': return 'secretary_review';
      default: return 'self';
    }
  })();
  ...
  return <section className="card overflow-hidden">
    <header className="flex flex-wrap items-start justify-between gap-4 border-b border-[var(--border)] p-5">
      ...
      <div className="text-left"><p className="text-2xl font-black">{form.finalScore ?? form.criteria.reduce((sum, item) => sum + (item.effectiveScore ?? 0), 0)} / 100</p><p className="muted text-xs">الموعد: {form.cycle.effectiveDeadline ? new Date(form.cycle.effectiveDeadline).toLocaleString('ar-EG') : '—'}</p>{form.parallelFlow ? <p className="mt-1 text-xs font-bold text-brand">مسار V23 المتوازي</p> : null}<button className="btn-secondary mt-2" onClick={onDone}>إغلاق التفاصيل</button></div>
    </header>

    {/* V23: مؤشر تقدم المراجعة المتوازية */}
    {form.currentStage === 'parallel_review' ? <div className="flex items-center gap-4 border-b border-[var(--border)] bg-[var(--surface-muted)] px-5 py-3 text-sm">
      <span className="font-bold">تقدم المراجعة المتوازية:</span>
      <span className={form.hrCompleted ? 'text-green-700 font-bold' : 'text-amber-600'}>HR {form.hrCompleted ? '✓ اكتمل' : '⏳ جارٍ'}</span>
      <span className={form.managerCompleted ? 'text-green-700 font-bold' : 'text-amber-600'}>المدير {form.managerCompleted ? '✓ اكتمل' : '⏳ جارٍ'}</span>
    </div> : null}
    ...
    <div className="p-5">
      <nav className="mb-5 flex flex-wrap gap-1 rounded-xl bg-[var(--surface-muted)] p-1" role="tablist">
        ...
        {showCompliance ? <button role="tab" aria-selected={tab === 'compliance'} className={tabCls(tab === 'compliance')} onClick={() => setTab('compliance')}>الالتزام</button> : null}
        {showSession ? <button role="tab" aria-selected={tab === 'session'} className={tabCls(tab === 'session')} onClick={() => setTab('session')}>الجلسة</button> : null}
      </nav>
      ...
      <div className="space-y-6">
        ...
        {form.editableStage ? <section className="rounded-2xl border border-[var(--border)] p-4"><label className="text-sm font-bold">ملاحظة المرحلة<textarea className="input mt-2 min-h-20" value={note} onChange={(event) => setNote(event.target.value)} /></label><div className="mt-3 flex flex-wrap gap-2">{form.editableStage !== 'self' ? <button className="btn-secondary" disabled={pending || note.trim().length < 5} onClick={() => { void commands.returnStage.mutateAsync({ p_evaluation_id: form.id, p_target_stage: returnTarget, p_note: note }); }}>إعادة للتصحيح</button> : null}<button className="btn-primary" disabled={pending} onClick={() => void submit()}><CheckCircle2 className="size-4" />{submitLabel}</button></div></section> : null}
      </div>
    </div>
  </section>;
}
