    ...
    <div className="space-y-5">
      ...
      {catalog.isLoading ? (
        ...
        <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          <MetricCard label="رحلات قيد التنفيذ" value={inProgress} icon={PackageCheck} hint="في مرحلة التهيئة حالياً" />
          <MetricCard label="رحلات مكتملة" value={completed} icon={CheckCircle2} hint="اجتازت مرحلة التهيئة" />
          <MetricCard label="متوسط الإنجاز" value={`${avgProgress}%`} icon={Award} hint="عبر جميع الرحلات" />
          <MetricCard label="موظفون في التهيئة" value={onboardingEmployees} icon={UsersRound} hint="من قائمة الموظفين المتاحة" />
        </section>
      )}
