...
export function KnowledgeHubPage() {
  ...
  const tab: TabKey = TABS.some((t) => t.key === raw) ? (raw as TabKey) : 'knowledge';
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap gap-1 rounded-xl border border-[var(--border)] p-1" role="tablist" aria-label="أقسام التدريب والمعرفة">
        {TABS.map((t) => (
          <button
            key={t.key}
            type="button"
