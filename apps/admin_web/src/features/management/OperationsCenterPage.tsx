        ...
        <section className="card overflow-hidden">
          <div className="hidden overflow-x-auto md:block">
            <table className="w-full min-w-[880px] text-right text-sm">
              ...
              <tbody>{tasks.map((item) => <tr className="border-t border-[var(--border)]" key={item.id}><td className="p-4"><strong>{item.title}</strong>{item.description ? <p className="muted mt-1 max-w-md text-xs">{item.description}</p> : null}</td><td className="p-4">{item.assigneeName}</td><td className="p-4"><StatusBadge value={item.priority} /></td><td className="p-4">{date(item.dueDate)}</td><td className="p-4"><StatusBadge value={item.status} /></td><td className="p-4"><TaskAction id={item.id} status={item.status} pending={commands.transitionTask.isPending} transition={(status) => void commands.transitionTask.mutateAsync({ id: item.id, status })} /></td></tr>)}</tbody>
            </table>
          </div>
          <div className="divide-y divide-[var(--border)] md:hidden">{tasks.map((item) => <article className="space-y-3 p-5" key={item.id}><div className="flex items-start justify-between gap-3"><div><strong>{item.title}</strong><p className="muted mt-1 text-xs">{item.assigneeName} · {date(item.dueDate)}</p></div><StatusBadge value={item.priority} /></div><div className="flex items-center justify-between gap-3"><StatusBadge value={item.status} /><TaskAction id={item.id} status={item.status} pending={commands.transitionTask.isPending} transition={(status) => void commands.transitionTask.mutateAsync({ id: item.id, status })} /></div></article>)}</div>
          {!tasks.length ? <EmptyState title="لا توجد مهام مطابقة" description="أنشئ مهمة جديدة أو غيّر عبارة البحث." /> : null}
        </section>
      ) : null}

      {data && tab === 'missions' ? (
        <section className="grid gap-4 lg:grid-cols-2">{missions.map((item) => <article className="card p-5" key={item.id}><div className="flex items-start justify-between gap-3"><div><h2 className="font-black">{item.destination}</h2><p className="muted mt-1 text-sm">{item.employeeName}</p></div><StatusBadge value={item.status} /></div><p className="mt-4 leading-7">{item.purpose}</p><div className="mt-4 grid grid-cols-2 gap-3 text-sm"><Info label="الفترة" value={`${date(item.startAt, true)} — ${date(item.endAt, true)}`} /><Info label="وسيلة الانتقال" value={transport(item.transportMode)} /></div></article>)}{!missions.length ? <div className="lg:col-span-2"><EmptyState title="لا توجد مأموريات مطابقة" description="تظهر المأموريات بعد إنشائها من مركز الطلبات." /></div> : null}</section>
      ) : null}

      {data && tab === 'convoys' ? (
        <section className="grid gap-4 lg:grid-cols-2">{convoys.map((item) => <article className="card p-5" key={item.id}><div className="flex items-start justify-between gap-3"><div><h2 className="font-black">{item.name}</h2><p className="muted mt-1 text-sm">المسؤول: {item.employeeName}</p></div><StatusBadge value={item.status} /></div><div className="mt-5 flex items-center gap-3 rounded-2xl bg-[var(--surface-muted)] p-4"><span className="rounded-xl bg-[var(--surface)] p-2"><BusFront className="size-5 text-[var(--brand-primary)]" /></span><div><strong>{item.origin} ← {item.destination}</strong><p className="muted mt-1 text-xs">التحرك {date(item.departureAt, true)}</p></div></div><div className="mt-4 grid grid-cols-2 gap-3 text-sm"><Info label="الركاب" value={`${item.passengers} فرد`} /><Info label="المركبات" value={`${item.vehicles} مركبة`} /></div></article>)}{!convoys.length ? <div className="lg:col-span-2"><EmptyState title="لا توجد قوافل مطابقة" description="تظهر القوافل بعد إنشائها واعتمادها من مسار الطلبات." /></div> : null}</section>
      ) : null}
