  ...
  const connectors = useMemo(
    () =>
      (data?.integrations ?? []).filter(
        (item) => !term || `${item.name} ${item.provider} ${item.category} ${item.status}`.toLocaleLowerCase('ar').includes(term),
      ),
    [data, term],
  );
  const outbox = useMemo(
    () => (data?.outbox ?? []).filter((item) => !term || `${item.eventType} ${item.status} ${item.error ?? ''}`.toLocaleLowerCase('ar').includes(term)),
    [data, term],
  );
  const logs = useMemo(
    () =>
      (data?.logs ?? []).filter(
        (item) => !term || `${item.operation ?? ''} ${item.status} ${item.direction} ${item.error ?? ''}`.toLocaleLowerCase('ar').includes(term),
      ),
    [data, term],
  );
  const runs = useMemo(
    () => (data?.automationRuns ?? []).filter((item) => !term || `${item.status} ${item.error ?? ''}`.toLocaleLowerCase('ar').includes(term)),
    [data, term],
  );
      ...
      {!query.isError && query.isLoading ? (
        <section className="grid gap-4 lg:grid-cols-2 xl:grid-cols-3" aria-label="جارٍ تحميل التكاملات">
          {Array.from({ length: 6 }).map((_, index) => (
            <SkeletonCard key={index} className="h-52 p-5" />
          ))}
        </section>
      ) : null}
        ...
        <section className="grid gap-4 lg:grid-cols-2 xl:grid-cols-3">
          {connectors.map((item) => (
            <article className="card p-5" key={item.id}>
              <div className="flex items-start justify-between gap-3">
                <span
                  className={`rounded-2xl p-3 ${item.status === 'error' ? 'bg-[var(--danger-soft)] text-[var(--danger)]' : 'bg-[var(--surface-muted)] text-[var(--brand-primary)]'}`}
                >
                  <ServerCog className="size-6" aria-hidden="true" />
                </span>
                <StatusBadge value={item.status} />
              </div>
              <h2 className="mt-5 font-black">{item.name}</h2>
              <p className="muted mt-1 text-sm">
                {item.provider} · {item.category}
              </p>
              <div className="mt-4 rounded-xl bg-[var(--surface-muted)] p-3 text-xs">
                <span className="muted">آخر مزامنة</span>
          ...
          {!connectors.length ? (
            <div className="lg:col-span-2 xl:col-span-3">
              <EmptyState title="لا توجد موصلات مطابقة" description="غيّر عبارة البحث أو أضف الموصل من إعدادات البيئة." />
            </div>
          ) : null}
        </section>
      ) : null}
              ...
              <tbody>
                {outbox.map((item) => (
                  <tr key={item.id} className="border-t border-[var(--border)]">
                    <td className="p-4">
                      <strong className="font-mono text-xs">{item.eventType}</strong>
                      <p className="muted mt-1 text-xs">أُنشئ {date(item.createdAt)}</p>
                    </td>
                    <td className="p-4">
                      <StatusBadge value={item.status} />
                    </td>
                    ...
                    <td className="p-4 whitespace-nowrap">{date(item.nextAttemptAt)}</td>
                    <td className="max-w-sm p-4 text-xs text-[var(--danger)]">{item.error ?? '—'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
            ...
            {outbox.map((item) => (
              <article key={item.id} className="p-5">
                ...
                <p className="muted mt-3 text-xs">
                  محاولات {item.attempts}/{item.maxAttempts} · التالي {date(item.nextAttemptAt)}
                </p>
                {item.error ? <p className="mt-3 text-sm font-bold text-[var(--danger)]">{item.error}</p> : null}
              </article>
            ))}
          </div>
      ...
      {data && tab === 'logs' ? (
        <section className="card overflow-hidden">
          <div className="divide-y divide-[var(--border)]">
            {logs.map((item) => (
              <article key={item.id} className="flex flex-col gap-3 p-5 lg:flex-row lg:items-center lg:justify-between">
                <div className="flex min-w-0 items-start gap-3">
                  <span
                    className={`rounded-xl p-2.5 ${item.status === 'success' ? 'bg-[var(--success-soft)] text-[var(--success)]' : item.status === 'failed' ? 'bg-[var(--danger-soft)] text-[var(--danger)]' : 'bg-[var(--surface-muted)]'}`}
                  >
                    {item.status === 'success' ? (
                  ...
                  <div>
                    <div className="flex flex-wrap items-center gap-2">
                      <strong>{item.operation ?? 'عملية تكامل'}</strong>
                      <StatusBadge value={item.status} />
                    ...
                    {item.error ? <p className="mt-2 text-sm font-bold text-[var(--danger)]">{item.error}</p> : null}
                  </div>
                </div>
              </article>
            ))}
        ...
        <section className="grid gap-4 lg:grid-cols-2">
          {runs.map((item) => (
            <article key={item.id} className="card p-5">
              <div className="flex items-start justify-between gap-3">
                <span className="rounded-xl bg-[var(--surface-muted)] p-2.5">
                  <Bot className="size-5" aria-hidden="true" />
                </span>
                <StatusBadge value={item.status} />
              </div>
              <h2 className="mt-4 font-black">تشغيل أتمتة</h2>
              <p className="muted mt-1 font-mono text-xs">{item.id}</p>
              <div className="mt-4 flex items-center justify-between gap-3 text-xs">
                <span className="inline-flex items-center gap-2">
                  <Clock3 className="size-4" aria-hidden="true" />
                  بدأ {date(item.createdAt)}
                </span>
                <span>{item.attempts} محاولة</span>
              </div>
              {item.error ? <p className="mt-3 rounded-xl bg-[var(--danger-soft)] p-3 text-sm font-bold text-[var(--danger)]">{item.error}</p> : null}
            </article>
          ))}
          {!runs.length ? (
            <div className="lg:col-span-2">
              <EmptyState title="لا توجد تشغيلات مطابقة" description="ستظهر نتائج قواعد الأتمتة هنا." />
            </div>
          ) : null}
        </section>
      ) : null}
...
function TabButton({ active, onClick, label }: { active: boolean; onClick: () => void; label: string }) {
  return (
    <button type="button" role="tab" aria-selected={active} className={`filter-chip ${active ? 'is-active' : ''}`} onClick={onClick}>
      {label}
    </button>
  );
}
