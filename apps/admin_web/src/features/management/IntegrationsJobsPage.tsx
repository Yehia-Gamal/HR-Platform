...
export function IntegrationsJobsPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {commands.toggle.isError ? <ErrorBanner message={safeErrorMessage(commands.toggle.error)} /> : null}
      ...
      {data && tab === 'logs' ? (
        <section className="card overflow-hidden">
          <div className="divide-y divide-[var(--border)]">{logs.map((item) => <article key={item.id} className="flex flex-col gap-3 p-5 lg:flex-row lg:items-center lg:justify-between"><div className="flex min-w-0 items-start gap-3"><span className={`rounded-xl p-2.5 ${item.status === 'success' ? 'bg-[var(--success-soft)] text-[var(--success)]' : item.status === 'failed' ? 'bg-[var(--danger-soft)] text-[var(--danger)]' : 'bg-[var(--surface-muted)]'}`}>{item.status === 'success' ? <CheckCircle2 className="size-5" aria-hidden="true" /> : item.status === 'failed' ? <CircleOff className="size-5" aria-hidden="true" /> : <Activity className="size-5" aria-hidden="true" />}</span><div><div className="flex flex-wrap items-center gap-2"><strong>{item.operation ?? 'عملية تكامل'}</strong><StatusBadge value={item.status} /><StatusBadge value={item.direction} /></div><p className="muted mt-2 text-xs">{date(item.occurredAt)}{item.httpStatus ? ` · HTTP ${item.httpStatus}` : ''}{item.durationMs !== null ? ` · ${item.durationMs}ms` : ''}</p>{item.error ? <p className="mt-2 text-sm font-bold text-[var(--danger)]">{item.error}</p> : null}</div></div></article>)}</div>
          {!logs.length ? <EmptyState title="لا توجد سجلات مطابقة" description="ستظهر اتصالات الموصلات هنا." /> : null}
        </section>
      ) : null}

      {data && tab === 'automations' ? (
        <section className="grid gap-4 lg:grid-cols-2">{runs.map((item) => <article key={item.id} className="card p-5"><div className="flex items-start justify-between gap-3"><span className="rounded-xl bg-[var(--surface-muted)] p-2.5"><Bot className="size-5" aria-hidden="true" /></span><StatusBadge value={item.status} /></div><h2 className="mt-4 font-black">تشغيل أتمتة</h2><p className="muted mt-1 font-mono text-xs">{item.id}</p><div className="mt-4 flex items-center justify-between gap-3 text-xs"><span className="inline-flex items-center gap-2"><Clock3 className="size-4" aria-hidden="true" />بدأ {date(item.createdAt)}</span><span>{item.attempts} محاولة</span></div>{item.error ? <p className="mt-3 rounded-xl bg-[var(--danger-soft)] p-3 text-sm font-bold text-[var(--danger)]">{item.error}</p> : null}</article>)}{!runs.length ? <div className="lg:col-span-2"><EmptyState title="لا توجد تشغيلات مطابقة" description="ستظهر نتائج قواعد الأتمتة هنا." /></div> : null}</section>
      ) : null}
    </div>
  );
}
