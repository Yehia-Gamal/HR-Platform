  ...
  return (
    <div className="space-y-6">
      ...
      <section className="filter-bar flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        ...
        <label className="relative w-full lg:max-w-sm"><Search className="pointer-events-none absolute right-3 top-3 size-4 text-[var(--text-muted)]" /><input className="input pr-10" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="بحث في القسم الحالي…" aria-label="البحث في التكاملات" /></label>
      </section>
