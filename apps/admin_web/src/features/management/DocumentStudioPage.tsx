import { FileCheck2, FilePlus2, PenLine, X } from 'lucide-react';
...
export function DocumentStudioPage() {
  ...
  return <div className="space-y-6"><PageHeader title="استوديو المستندات والتوقيعات" description="قوالب Versioned، مستندات مولدة، وسلسلة توقيع متعددة الأطراف مرتبطة بنسخة المحتوى." actions={<button className="btn-primary" onClick={() => setOpen(true)}><FilePlus2 className="size-4" aria-hidden="true" />قالب جديد</button>} />
    ...
    <article className="card overflow-hidden"><div className="border-b border-[var(--border)] p-5"><h2 className="font-black">المستندات المولدة</h2></div><div className="divide-y divide-[var(--border)]">{data.documents.length ? data.documents.map(d => <div className="flex justify-between gap-4 p-5" key={d.id}><div><p className="font-black">{d.title}</p><p className="muted text-sm">{d.referenceNumber} · {d.employeeName ?? 'عام'}</p></div><div className="text-end"><StatusBadge value={d.status} /><p className="muted mt-1 text-xs">معلق {d.pendingSignatures}</p></div></div>) : <EmptyState title="لا توجد مستندات" description="سيظهر هنا ما يتم توليده من القوالب." />}</div></article></section></>}
    {open ? <div className="fixed inset-0 z-50 grid place-items-center p-4" style={{ background: 'color-mix(in srgb, var(--text-primary) 55%, transparent)' }}><form role="dialog" aria-modal="true" aria-labelledby="doc-studio-modal-title" className="card max-h-[92vh] w-full max-w-3xl space-y-4 overflow-y-auto p-6" onSubmit={submit}><div className="flex justify-between"><h2 id="doc-studio-modal-title" className="text-xl font-black">قالب جديد</h2><button type="button" className="icon-button" aria-label="إغلاق" onClick={() => setOpen(false)}><X className="size-4" aria-hidden="true" /></button></div>{commands.upsertTemplate.error ? <ErrorBanner message={commands.upsertTemplate.error instanceof Error ? commands.upsertTemplate.error.message : 'تعذر حفظ القالب'} /> : null}<div className="grid gap-4 sm:grid-cols-2"><Field label="الكود"><input className="input" required value={draft.code} onChange={e => setDraft({ ...draft, code: e.target.value })} /></Field><Field label="الاسم"><input className="input" required value={draft.name} onChange={e => setDraft({ ...draft, name: e.target.value })} /></Field></div><Field label="النوع"><select className="input" aria-label="النوع" value={draft.documentType} onChange={e => setDraft({ ...draft, documentType: e.target.value })}>{DOCUMENT_TYPES.map(dt => <option key={dt.value} value={dt.value}>{dt.label}</option>)}</select></Field><Field label="نص القالب"><textarea className="input min-h-56 font-mono" required value={draft.body} onChange={e => setDraft({ ...draft, body: e.target.value })} /></Field><div className="grid gap-3 sm:grid-cols-4">{SIGNATURE_ROLES.map(r => <label className="flex gap-2" key={r.key}><input type="checkbox" checked={draft[r.key]} onChange={e => setDraft({ ...draft, [r.key]: e.target.checked })} />{r.label}</label>)}</div><button className="btn-primary">حفظ القالب</button></form></div> : null}
  </div>;
}
