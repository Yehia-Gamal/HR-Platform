    ...
    <div className="space-y-6">
      ...
      <div className="location-mobile-switch" role="tablist" aria-label="تبويب العرض">
        <button
          ...
          className={`filter-chip ${tab === 'directory' ? 'is-active' : ''}`}
        ...
        <button
          ...
          className={`filter-chip ${tab === 'monitoring' ? 'is-active' : ''}`}
      ...
      {tab === 'monitoring' ? (
        ...
        <>
          ...
          {query.isError ? (
            <ErrorState title="تعذر تحميل دليل الموقع" description={safeErrorMessage(query.error)} onRetry={() => void query.refetch()} />
          ) : null}
...
function LocationMap({ items }: { items: LocationDirectoryItem[] }) {
  const points = items.filter(
    (item): item is LocationDirectoryItem & { lastLatitude: number; lastLongitude: number } => item.lastLatitude !== null && item.lastLongitude !== null,
  );
