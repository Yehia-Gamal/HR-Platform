      ...
      <section className="dashboard-hero">
        <div className="dashboard-hero-grid">
          <div>
            ...
            <h2 className="text-2xl font-black sm:text-3xl" title={auth.access?.displayName}>
          ...
          <div className="hero-actions">
            {quickActions.map((action) => {
              ...
              return (
                <Link key={action.to} to={action.to} className="hero-action">
        ...
        <>
            ...
            <section className="grid gap-4 xl:grid-cols-[1.2fr_.8fr]">
              <article className="card p-5">
                <div className="section-title-row">
                  <div>
                    <h2>نبض التشغيل</h2>
                    <p className="mt-1 text-xs text-[var(--text-muted)]">مؤشرات إرشادية تساعد على ترتيب العمل ولا تستبدل التقارير التفصيلية.</p>
                  </div>
                  ...
                  <div className="pulse-row" key={item.label}>
                    <div>
                      <div className="mb-2 flex items-center justify-between gap-3">
                        ...
                        <strong className="text-sm text-[var(--brand-primary)]">{item.value}%</strong>
                      </div>
                      <div
                    ...
                    <span className="grid size-9 place-items-center rounded-full bg-[var(--surface-muted)] text-xs font-black">{item.value}</span>
                  </div>
                ))}
              </article>
              ...
              <article className="card p-5">
                <div className="section-title-row">
                  <div>
                    <h2>أولويات اليوم</h2>
                    <p className="mt-1 text-xs text-[var(--text-muted)]">مرتبة وفق الوقت والأثر التشغيلي.</p>
                  </div>
                </div>
                {priorities.map((item) => (
                  <Link key={item.to + item.title} to={item.to} className="priority-item card-interactive">
                    <span className="priority-icon">
                      <Clock3 className="size-4" aria-hidden="true" />
                    </span>
                    <span className="min-w-0 flex-1">
                      <strong className="block text-sm">{item.title}</strong>
                      <small className="mt-1 block leading-5 text-[var(--text-muted)]">{item.description}</small>
                    </span>
          ...
          {type === 'hr' && data ? (
            <section className="card p-5">
              <div className="section-title-row">
                <div>
                  <h2>توزيع القوى العاملة</h2>
                  <p className="mt-1 text-xs text-[var(--text-muted)]">نظرة سريعة على حالة الموظفين.</p>
                </div>
              </div>
          ...
          {data ? (
            <section className="card flex flex-col gap-4 p-4 sm:flex-row sm:items-center sm:justify-between">
              <div>
                <p className="text-sm font-black">نطاق العرض</p>
