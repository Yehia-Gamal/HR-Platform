...
import {
  ...
  useCreateLeaveForEmployee,
  useGrantRestCreditBulk,
  useLeaveTypes,
} from './useLeaveTools';
      ...
      <div className="max-h-56 space-y-1 overflow-y-auto rounded-xl border border-[var(--border)] p-1.5">
        {employees.map((emp) => {
          ...
          return (
            ...
            >
              <input type="checkbox" checked={checked} onChange={() => onToggle(emp.id)} disabled={!emp.isActive} className="size-4 shrink-0 accent-[var(--brand-primary)]" />
    ...
    <form onSubmit={(e) => void submit(e)} className="card space-y-4 p-5">
      ...
      <div className="grid grid-cols-2 gap-3">
        <label className="block">
          ...
          <input type="date" className="input-field w-full text-sm" value={workDate} onChange={(e) => setWorkDate(e.target.value)} required disabled={grant.isPending} />
        </label>
        <label className="block">
    ...
    <form onSubmit={(e) => void submit(e)} className="card space-y-4 p-5">
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <label className="block">
          ...
          <input type="number" className="input-field w-full text-sm" value={year} min={2020} max={2100} onChange={(e) => setYear(Number(e.target.value))} required disabled={adjust.isPending} />
        </label>
        <label className="block">
          <span className="mb-1.5 block text-xs font-semibold text-[var(--text-muted)]">الكمية (بالأيام، +/−)</span>
          <input type="number" className="input-field w-full text-sm" value={units} onChange={(e) => setUnits(Number(e.target.value))} required disabled={adjust.isPending} step="0.5" />
        </label>
      ...
      <label className="block">
        ...
        <textarea className="input-field w-full resize-none text-sm" rows={2} minLength={5} value={reason} onChange={(e) => setReason(e.target.value)} placeholder="سبب التعديل..." required disabled={adjust.isPending} />
      </label>
    ...
    <form onSubmit={(e) => void submit(e)} className="card space-y-4 p-5">
      ...
      <div className="grid grid-cols-2 gap-3">
        <label className="block">
          ...
          <input type="date" className="input-field w-full text-sm" value={startDate} onChange={(e) => setStartDate(e.target.value)} required disabled={create.isPending} />
        </label>
        <label className="block">
          <span className="mb-1.5 block text-xs font-semibold text-[var(--text-muted)]">إلى تاريخ</span>
          <input type="date" className="input-field w-full text-sm" value={endDate} onChange={(e) => setEndDate(e.target.value)} required disabled={create.isPending} />
        </label>
      </div>

      <label className="block">
        <span className="mb-1.5 block text-xs font-semibold text-[var(--text-muted)]">السبب</span>
        <textarea className="input-field w-full resize-none text-sm" rows={2} minLength={3} value={reason} onChange={(e) => setReason(e.target.value)} placeholder="سبب الإجازة..." required disabled={create.isPending} />
      </label>

      <label className="block">
        <span className="mb-1.5 block text-xs font-semibold text-[var(--text-muted)]">ملاحظات التسليم (اختياري)</span>
        <input className="input-field w-full text-sm" value={handoverNotes} onChange={(e) => setHandoverNotes(e.target.value)} placeholder="من سيغطي العمل أثناء الإجازة؟" disabled={create.isPending} />
      </label>

      <button type="submit" className="btn-primary w-full" disabled={create.isPending || !selectedEmployee || !startDate || !endDate || reason.trim().length < 3}>
    ...
    <form onSubmit={(e) => void submit(e)} className="card space-y-4 p-5">
      ...
      <div className="grid grid-cols-2 gap-3">
        <label className="block">
          ...
          <div className="relative">
            <select value={assignmentType} onChange={(e) => setAssignmentType(e.target.value as 'CONVOY' | 'FUNDRAISING')} className="input-field w-full appearance-none text-sm">
        ...
        <label className="block">
          ...
          <input className="input-field w-full text-sm" value={title} onChange={(e) => setTitle(e.target.value)} minLength={3} placeholder="مثال: قافلة رمضان — حي الأمل" required disabled={create.isPending} />
        </label>
        <label className="block">
          <span className="mb-1.5 block text-xs font-semibold text-[var(--text-muted)]">من</span>
          <input type="datetime-local" className="input-field w-full text-sm" value={startAt} onChange={(e) => setStartAt(e.target.value)} required disabled={create.isPending} />
        </label>
        <label className="block">
          <span className="mb-1.5 block text-xs font-semibold text-[var(--text-muted)]">إلى</span>
          <input type="datetime-local" className="input-field w-full text-sm" value={endAt} onChange={(e) => setEndAt(e.target.value)} required disabled={create.isPending} />
        </label>
        <label className="block">
          <span className="mb-1.5 block text-xs font-semibold text-[var(--text-muted)]">الموقع (اختياري)</span>
          <input className="input-field w-full text-sm" value={location} onChange={(e) => setLocation(e.target.value)} placeholder="مثال: شارع المدارس" disabled={create.isPending} />
        </label>
        {assignmentType === 'FUNDRAISING' && (
          <label className="block">
            <span className="mb-1.5 block text-xs font-semibold text-[var(--text-muted)]">الهدف (جنيه، اختياري)</span>
            <input type="number" className="input-field w-full text-sm" value={targetAmount} min={0} onChange={(e) => setTargetAmount(e.target.value)} placeholder="مثال: 50000" disabled={create.isPending} />
          </label>
        )}
      </div>

      <div className="flex items-center gap-3 rounded-xl border border-[var(--border)] p-3">
        <input type="checkbox" id="use-all-active" className="size-4 accent-[var(--brand-primary)]" checked={useAllActive} onChange={(e) => setUseAllActive(e.target.checked)} />
      ...
      >
        {create.isPending ? 'جارٍ الإنشاء...' : `إنشاء ${assignmentType === 'CONVOY' ? 'القافلة' : 'الفاندي'} لـ ${useAllActive ? activeIds.length : selected.size} موظف`}
      </button>
    </form>
  );
}
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-start gap-2.5 rounded-xl border border-[var(--border)] bg-[var(--surface-raised)] p-4 text-sm">
        ...
        <p className="text-[var(--text-muted)]">
          الإجازة المرضية <strong className="text-[var(--text)]">مفتوحة بلا رصيد</strong> — يُترك القرار للمدير المباشر ثم عمليات 1 (أبو عمار) للموافقة أو الرفض.
        </p>
      </div>
