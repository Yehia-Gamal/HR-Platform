import { adminDeviceSchema, pendingDeviceSchema, type AdminDevice, type PendingDevice } from '@ahla/shared-contracts';
export type { AdminDevice, PendingDevice };
import { useMutation, useQuery, useQueryClient } from '@tanstack/react-query';
import { z } from 'zod';
import { rpc } from '../../core/rpc';
import { useAuth } from '../auth/AuthProvider';

const pendingDeviceArraySchema = z.array(pendingDeviceSchema);
const adminDeviceArraySchema = z.array(adminDeviceSchema);
...
export function useDeviceApprovals() {
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<PendingDevice[]> => {
      ...
      const data = await rpc('get_pending_devices_admin', undefined, pendingDeviceArraySchema);
      return data ?? [];
    },
  });
}
