...
export function DocumentsStudioPage() {
  ...
  if (catalogQuery.isError) return <ErrorState title="تعذّر التحميل" description={safeErrorMessage(catalogQuery.error)} onRetry={() => catalogQuery.refetch()} />;
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex gap-2">
        <button className={`btn-sm ${tab === 'documents' ? 'btn-primary' : 'btn-ghost'}`} onClick={() => setTab('documents')}>المستندات</button>
        <button className={`btn-sm ${tab === 'templates' ? 'btn-primary' : 'btn-ghost'}`} onClick={() => setTab('templates')}>القوالب</button>
      </div>
              ...
              <tbody className="divide-y divide-[var(--border-color)]">
                {filteredDocs.map((d) => (
                  <tr key={d.id} className="hover:bg-[var(--surface-hover)]">
                    ...
                    <td className="px-4 py-3"><StatusBadge status={DOC_STATUS_TONE[d.status] ?? 'neutral'} label={DOC_STATUS_LABELS[d.status] ?? d.status} /></td>
                    <td className="px-4 py-3 text-center">{d.pendingSignatures > 0 ? <span className="font-bold text-amber-600">{d.pendingSignatures}</span> : '—'}</td>
                  ...
                  <tr key={t.id} className="hover:bg-[var(--surface-hover)]">
                    ...
                    <td className="px-4 py-3 text-xs">
                      <div className="flex gap-1 flex-wrap">
                        ...
                        {!t.requiresEmployeeSignature && !t.requiresManagerSignature && !t.requiresHrSignature && !t.requiresExecutiveSignature && <span className="text-[var(--text-tertiary)]">لا توجد</span>}
                      </div>
                    </td>
                    <td className="px-4 py-3"><StatusBadge status={t.active ? 'success' : 'neutral'} label={t.active ? 'نشط' : 'معطل'} /></td>
                  </tr>
                ))}
              </tbody>
            </table>
...
function TemplateForm({ onSubmit, onCancel, isSaving }: {
    ...
    <form onSubmit={(e) => { e.preventDefault(); onSubmit(form); }} className="space-y-4">
      <div className="grid grid-cols-2 gap-4">
        <div>
          ...
          <input type="text" value={form.code} onChange={(e) => setForm({ ...form, code: e.target.value.toUpperCase() })} className="form-input w-full" placeholder="DOC-001" required />
        </div>
        <div>
      ...
      <div>
        ...
        <input type="text" value={form.name_ar} onChange={(e) => setForm({ ...form, name_ar: e.target.value })} className="form-input w-full" placeholder="عقد عمل جديد" required />
      </div>
      <div>
        <label className="label-field">محتوى القالب</label>
        <textarea value={form.body_template} onChange={(e) => setForm({ ...form, body_template: e.target.value })} className="form-textarea w-full min-h-[120px]" placeholder="اكتب محتوى المستند هنا…" />
      </div>
      <div className="flex flex-wrap gap-3 rounded-lg border border-[var(--border-color)] bg-[var(--surface-muted)] p-3">
        <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={form.requires_employee} onChange={(e) => setForm({ ...form, requires_employee: e.target.checked })} className="form-checkbox" /> توقيع الموظف</label>
        <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={form.requires_manager} onChange={(e) => setForm({ ...form, requires_manager: e.target.checked })} className="form-checkbox" /> توقيع المدير</label>
        <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={form.requires_hr} onChange={(e) => setForm({ ...form, requires_hr: e.target.checked })} className="form-checkbox" /> توقيع HR</label>
        <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={form.requires_executive} onChange={(e) => setForm({ ...form, requires_executive: e.target.checked })} className="form-checkbox" /> توقيع تنفيذي</label>
        <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={form.active} onChange={(e) => setForm({ ...form, active: e.target.checked })} className="form-checkbox" /> نشط</label>
      </div>
      <div className="flex justify-end gap-2 border-t border-[var(--border-color)] pt-4">
        ...
        <button type="submit" disabled={!canSubmit || isSaving} className="btn-primary">{isSaving ? 'جاري الحفظ…' : 'حفظ القالب'}</button>
      </div>
    </form>
  );
}
