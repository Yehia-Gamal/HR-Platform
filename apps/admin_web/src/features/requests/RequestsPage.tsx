...
export function RequestsPage() {
  ...
  return <div className="space-y-6">
    ...
    {/* ─── تصحيحات الحضور ─── */}
    {typeTab === 'corrections' ? <section className="card p-5">
      ...
      {correctionsQuery.isLoading ? <ListSkeleton rows={3} /> : corrections.length === 0 ? <EmptyState title="لا توجد تصحيحات" description="لا توجد طلبات تصحيح في الشهر الحالي." /> : <div className="mt-4 space-y-3">{corrections.map((item) => <article key={item.id} className="rounded-2xl border border-[var(--border)] p-4">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <div className="flex items-center gap-2">
            <UserAvatar displayName={item.employeeName} size="sm" />
            <strong>{item.employeeName}</strong>
            <p className="muted text-sm">{item.workDate} · {item.type} · {item.reason}</p>
          </div>
    ...
    {selected ? createPortal(<div className="fixed inset-0 z-50 grid place-items-center bg-black/50 p-4" onClick={() => setSelected(null)}><section className="card w-full max-w-xl p-6" role="dialog" aria-modal="true" aria-labelledby="decision-title" aria-describedby="decision-reason" onClick={(e) => e.stopPropagation()}><div className="flex items-start justify-between gap-4"><div><p className="text-xs font-black text-brand">طلب #{selected.requestNumber}</p><h2 id="decision-title" className="mt-1 text-xl font-black">{selected.title || labels[selected.requestType]}</h2><div className="mt-1 flex items-center gap-2"><UserAvatar displayName={selected.employeeName} size="sm" /><p className="muted text-sm">{selected.employeeName}</p></div></div><button className="icon-button" onClick={() => setSelected(null)} aria-label="إغلاق"><X className="size-5" aria-hidden="true" /></button></div><p id="decision-reason" className="mt-5 rounded-2xl bg-[var(--surface-muted)] p-4 text-sm leading-7">{selected.reason || 'لا يوجد سبب تفصيلي.'}</p><label className="mt-5 block text-sm font-bold">ملاحظة القرار<textarea className="input mt-2 min-h-28 resize-y" value={comment} onChange={(e) => setComment(e.target.value)} placeholder="الرفض يتطلب سببًا واضحًا، والموافقة يمكن أن تتضمن ملاحظة." /></label><p id="reject-hint" className="muted mt-2 text-xs">يتطلب الرفض إدخال سبب لا يقل عن ٣ أحرف.</p>{decision.isError ? <div className="mt-3"><ErrorBanner message={decision.error instanceof Error ? decision.error.message : 'تعذر تنفيذ الإجراء'} /></div> : null}<div className="mt-5 grid gap-3 sm:grid-cols-2"><button className="inline-flex items-center justify-center gap-2 rounded-xl px-4 py-3 font-black text-white disabled:opacity-50" style={{ background: 'var(--success)' }} disabled={decision.isPending} onClick={() => void submitDecision('approve')}><Check className="size-5" aria-hidden="true" />اعتماد</button><button className="inline-flex items-center justify-center gap-2 rounded-xl px-4 py-3 font-black text-white disabled:opacity-50" style={{ background: 'var(--danger)' }} aria-describedby="reject-hint" disabled={decision.isPending || comment.trim().length < 3} onClick={() => void submitDecision('reject')}><X className="size-5" aria-hidden="true" />رفض مع السبب</button></div></section></div>, document.body) : null}
  </div>;
}
