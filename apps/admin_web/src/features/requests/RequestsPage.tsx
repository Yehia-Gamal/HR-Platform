...
const labels: Record<RequestSummary['requestType'], string> = { leave: 'إجازة', mission: 'مأمورية', convoy: 'قافلة', late_permit: 'إذن حضور', early_permit: 'إذن انصراف', attendance_correction: 'تصحيح حضور' };
...
export function RequestsPage() {
  ...
  return <div className="space-y-6">
    ...
    {typeTab === 'corrections' ? <section className="card p-5">
      ...
      {correctionsCommands.decideCorrection.isError ? <div className="mt-3"><ErrorBanner message={safeErrorMessage(correctionsCommands.decideCorrection.error)} /></div> : null}
    ...
    {query.isError ? <ErrorState description={safeErrorMessage(query.error)} onRetry={() => void query.refetch()} />
    ...
    {selected ? <DialogOverlay title={`طلب #${selected.requestNumber} — ${selected.title || labels[selected.requestType]}`} onClose={() => setSelected(null)} maxWidth="max-w-xl"><div className="flex items-center gap-2 mb-5"><UserAvatar displayName={selected.employeeName} size="sm" /><p className="muted text-sm">{selected.employeeName}</p></div><p id="decision-reason" className="rounded-2xl bg-[var(--surface-muted)] p-4 text-sm leading-7">{selected.reason || 'لا يوجد سبب تفصيلي.'}</p><label className="mt-5 block text-sm font-bold">ملاحظة القرار<textarea className="input mt-2 min-h-28 resize-y" value={comment} onChange={(e) => setComment(e.target.value)} placeholder="الرفض يتطلب سببًا واضحًا، والموافقة يمكن أن تتضمن ملاحظة." /></label><p id="reject-hint" className="muted mt-2 text-xs">يتطلب الرفض إدخال سبب لا يقل عن ٣ أحرف.</p>{decision.isError ? <div className="mt-3"><ErrorBanner message={safeErrorMessage(decision.error)} /></div> : null}<div className="mt-5 grid gap-3 sm:grid-cols-2"><button className="inline-flex items-center justify-center gap-2 rounded-xl px-4 py-3 font-black text-white disabled:opacity-50" style={{ background: 'var(--success)' }} disabled={decision.isPending} onClick={() => void submitDecision('approve')}><Check className="size-5" aria-hidden="true" />اعتماد</button><button className="inline-flex items-center justify-center gap-2 rounded-xl px-4 py-3 font-black text-white disabled:opacity-50" style={{ background: 'var(--danger)' }} aria-describedby="reject-hint" disabled={decision.isPending || comment.trim().length < 3} onClick={() => void submitDecision('reject')}><X className="size-5" aria-hidden="true" />رفض مع السبب</button></div></DialogOverlay> : null}
  </div>;
}
