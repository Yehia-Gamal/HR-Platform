...
const appSource = fs.readFileSync(path.resolve(__dirname, '../app/App.tsx'), 'utf-8');

const shellSource = fs.readFileSync(path.resolve(__dirname, '../features/workspaces/WorkspaceShell.tsx'), 'utf-8');
...
describe('V23 §13: الميزات المحذوفة', () => {
  ...
  it('§13.4: مسار العهد غير موجود في التوجيه', () => {
    const hasCustodyRoute = routePaths.some((p) => /\bcustody\b/i.test(p));
    expect(hasCustodyRoute).toBe(false);
  });

  it('§13.5: مسار نهاية العقد غير موجود في التوجيه', () => {
    const hasContractEndRoute = routePaths.some((p) => /\bcontract[_-]?end\b/i.test(p));
  ...
  it('§13.1/4/5: الميزات المحذوفة ليست في قائمة التنقل الجانبية', () => {
    ...
    const navTargets = [...shellSource.matchAll(/to:\s*['"]([^'"]+)['"]/g)].map((m) => m[1]);
