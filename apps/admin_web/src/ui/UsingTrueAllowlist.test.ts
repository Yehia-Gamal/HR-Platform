...
const allowlistSource = allowlistExists
...
describe('V23 §14.2: USING(true) allowlist موثقة', () => {
  ...
  it('§14.2.5: لا يوجد جدول حساس في القائمة', () => {
    ...
    const allowlistTableNames = [
      ...allowlistSource.matchAll(/\| `(\w+)` \|/g),
    ].map((m) => m[1]);
