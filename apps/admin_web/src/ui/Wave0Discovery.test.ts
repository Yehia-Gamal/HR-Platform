...
describe('V23 §15.1: Wave 0 — Discovery + baselines', () => {
  ...
  it('§15.1.3: مجلد اختبارات pgTAP موجود ويحتوي ≥48 ملف', () => {
    ...
    const sqlFiles = fs.readdirSync(testsDir).filter((f) => f.endsWith('.sql'));
  ...
  it('§15.1.4: مجلد migrations يحتوي ≥116 migration', () => {
    ...
    const sqlFiles = fs.readdirSync(migsDir).filter((f) => f.endsWith('.sql'));
  ...
  it('§15.1.6: 74 متطلب مُوثّق في مصفوفة التتبع', () => {
    const source = fs.readFileSync(path.join(root, 'TRACEABILITY.md'), 'utf-8');
  ...
  it('§15.1.7: لا يوجد متطلب بحالة DISCOVERED (جميعها تجاوزت الاستكشاف)', () => {
    const source = fs.readFileSync(path.join(root, 'TRACEABILITY.md'), 'utf-8');
    const requirementRows = source.split('\n').filter((line) => /^\| \d+\.\d+ \|/.test(line));
    const discoveredRows = requirementRows.filter((r) => r.includes('DISCOVERED'));
  ...
  it('§15.1.8: Edge Functions موجودة', () => {
    ...
    const fns = fs.readdirSync(fnDir).filter((f) => !f.startsWith('_') && fs.statSync(path.join(fnDir, f)).isDirectory());
  ...
  it('§15.1.10: Flutter mobile app موجود', () => {
    ...
    expect(fs.existsSync(path.join(flutterDir, 'pubspec.yaml'))).toBe(true);
  });
});
