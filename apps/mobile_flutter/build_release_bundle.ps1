...
Write-Host "Building signed production release App Bundle (AAB)..." -ForegroundColor Cyan
flutter build appbundle --release --dart-define-from-file="$defineFile"

if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

Write-Host "`nApp Bundle (AAB) Built successfully at build\app\outputs\bundle\release\app-release.aab" -ForegroundColor Green
