  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      ...
      body: RefreshIndicator(
        onRefresh: () async => ref.invalidate(myLocationRequestsProvider),
        child: requests.when(
            ...
            children: [
              ...
              Center(
                child: FilledButton.tonalIcon(
                  onPressed: () => ref.invalidate(myLocationRequestsProvider),
          ...
          data: (items) => items.isEmpty
              ...
              : ListView.separated(
                  ...
                  itemBuilder: (context, index) =>
                      _RequestCard(request: items[index], access: access),
                ),
        ),
      ),
    );
  ...
  Future<void> _respond(bool accept) async {
    ...
    try {
      ...
      if (widget.request.needsVideo) {
        ...
        if (id == null) throw StateError('لا يوجد ملف موظف مرتبط بالحساب.');
        await Navigator.push<bool>(
          context,
          MaterialPageRoute(
            builder: (_) =>
                VideoVerificationPage(request: widget.request, employeeId: id),
          ),
        );
        ...
        await Navigator.push<bool>(
          context,
          MaterialPageRoute(
            builder: (_) => LiveTrackingSessionPage(request: widget.request),
          ),
        );
      }
      ...
      if (mounted) {
        ...
        final isGpsOff = msg.contains('GPS') ||
            ...
            msg.contains('GpsDisabledException');
        setState(() {
          error = isGpsOff
              ? 'خدمة الموقع غير مفعلة. فعّل GPS ثم أعد المحاولة.'
              : 'حدث خطأ أثناء الاستجابة للطلب. أعد المحاولة.';
          _gpsOff = isGpsOff;
        });
      }
    } finally {
                ...
                children: [
                  ...
                  Expanded(
                    child: FilledButton(
                      ...
                      child: Text(busy ? 'جارٍ التنفيذ...' : 'موافقة ومشاركة'),
                    ),
                  ),
                ],
              ),
