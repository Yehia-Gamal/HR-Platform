...
/// تسميات وألوان موحدة لنظام القضايا — تُستخدم من جميع الصفحات
class DisputeLabels {
  DisputeLabels._();
  ...
  // ── Severity ───────────────────────────────────────────────
  static const severityLabels = <String, String>{
    'critical': 'حرجة',
    'urgent': 'عاجلة',
    'high': 'عالية',
    'medium': 'متوسطة',
  ...
  static const severityColors = <String, Color>{
  ...
  // ── Case type ──────────────────────────────────────────────
  static const caseTypeLabels = <String, String>{
    'complaint': 'شكوى',
    'grievance': 'تظلم',
    'disciplinary': 'تأديبي',
    'harassment': 'تحرش',
  ...
  // ── Helpers ────────────────────────────────────────────────
  static String statusLabel(String key) => statusLabels[key] ?? key;
  static Color statusColor(String key) => statusColors[key] ?? const Color(0xFF757575);
  static String severityLabel(String key) => severityLabels[key] ?? key;
  static Color severityColor(String key) => severityColors[key] ?? const Color(0xFF757575);
  static String caseTypeLabel(String key) => caseTypeLabels[key] ?? key;
  static String actionLabel(String key) => actionLabels[key] ?? key;
}
