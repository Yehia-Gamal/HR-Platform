...
describe('official KPI contracts', () => {
  it('accepts the HR and employee acknowledgement workflow stages', () => {
    expect(kpiEvaluationSummarySchema.shape.currentStage.parse('hr')).toBe('hr');
    expect(kpiEvaluationSummarySchema.shape.currentStage.parse('acknowledgement')).toBe('acknowledgement');
  });
  ...
  it('parses server-authored attendance and the seven official components', () => {
    ...
    const form = kpiEvaluationFormSchema.parse({
      ...
      periodMonth: '2026-07-01', currentStage: 'hr', workflowStatus: 'HR_EVALUATION_IN_PROGRESS', editableStage: 'hr', locked: false, finalScore: null, finalRating: null,
