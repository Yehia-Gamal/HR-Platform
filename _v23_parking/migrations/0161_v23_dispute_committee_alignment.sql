-- V23: Dispute committee alignment
-- 1. Add 'resolved_friendly' status to dispute_cases
-- 2. Add 'resolve_friendly' action to transition_dispute_case()
-- 3. Create submit_my_dispute_v23() simplified intake RPC
-- 4. Update get_committee_dispute_portal() summary for resolved_friendly
-- Depends on: 0059, 0131, 0152

begin;

-- ═══════════════════════════════════════════════════════════════════════════════
-- 1. Expand status check constraint to include 'resolved_friendly'
-- ═══════════════════════════════════════════════════════════════════════════════
...
-- ═══════════════════════════════════════════════════════════════════════════════
-- 2. Recreate transition_dispute_case with 'resolve_friendly' action
-- ═══════════════════════════════════════════════════════════════════════════════
...
begin
 ...
 if p_action in ('reject','request_more_information','escalate','return_to_committee','close','reopen','resolve_friendly','force_status') and length(trim(coalesce(p_reason,'')))<5 then raise exception 'REASON_REQUIRED' using errcode='22023'; end if;
 ...
 if v.actor_employee_id is not null and p_action in ('accept','reject','request_more_information','escalate','return_to_committee','resolve_friendly','close','reopen') then
  perform public.enqueue_dispute_notification(p_case_id,v.actor_employee_id,p_action||':actor',
   case p_action when 'accept' then 'تم قبول المشكلة للدراسة' when 'reject' then 'تم رفض المشكلة شكليًا' when 'request_more_information' then 'مطلوب استكمال بيانات المشكلة' when 'escalate' then 'تم تصعيد المشكلة' when 'return_to_committee' then 'أعيدت المشكلة إلى اللجنة' when 'resolve_friendly' then 'تم حل المشكلة وديًا' when 'close' then 'تم إغلاق المشكلة' else 'أعيد فتح المشكلة' end,
...
-- ═══════════════════════════════════════════════════════════════════════════════
-- 3. V23 simplified dispute intake RPC (mobile)
-- ═══════════════════════════════════════════════════════════════════════════════
...
begin
 return public.submit_my_dispute(
  p_title, p_description, p_case_type,
  'normal', null, null,
  p_parties, p_witnesses,
  null, null, null, null,
  true, p_truth_confirmed, p_confidentiality_accepted
 );
end $$;
...
-- ═══════════════════════════════════════════════════════════════════════════════
-- 4. Update committee portal summary to include resolved_friendly
-- ═══════════════════════════════════════════════════════════════════════════════
