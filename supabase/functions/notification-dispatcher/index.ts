...
/** صف اشتراك دفع من push_subscriptions. */
...
Deno.serve(
  createHandler({ functionName: 'notification-dispatcher', version: '1.0.0' }, async (req, ctx) => {
    ...
    for (const job of ordered) {
      ...
      if (job.channel === 'push' && notification?.entity_type === 'live_location_request' && notification.entity_id) {
        const { data: request } = await supabase.from('live_location_requests').select('status,expires_at').eq('id', notification.entity_id).maybeSingle();
        const stillPending = !!request && request.status === 'pending' && (!request.expires_at || new Date(request.expires_at).getTime() > Date.now());
        if (!stillPending) {
          await supabase
            .from('notification_jobs')
            .update({
      ...
      try {
        ...
        if (projectId && saJson && fcmTargets.length) {
          ...
          for (const sub of fcmTargets) {
            ...
            if (res.ok) {
              ...
              try {
                msgId = JSON.parse(txt)?.name ?? null;
              } catch {
                /* ignore */
              }
        ...
        const providerUrl = Deno.env.get('PUSH_PROVIDER_URL');
        const providerToken = Deno.env.get('PUSH_PROVIDER_TOKEN');
...
async function logDelivery(
  ...
  status: 'token_missing' | 'sent' | 'failed' | 'delivered',
...
async function encryptWebPushPayload(payload: string, p256dhBase64: string, authBase64: string): Promise<ArrayBuffer> {
  ...
  offset += 16;
  record.set(new Uint8Array([0]), offset);
  offset += 1; // rs = 0 (no padding)
  record.set(new Uint8Array([0]), offset);
  offset += 1; // id = 0 (direct)
  record.set(new Uint8Array(localPublicKey), offset);
  offset += 65;
