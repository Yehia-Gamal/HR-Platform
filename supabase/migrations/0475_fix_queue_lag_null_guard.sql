-- =====================================================================
-- 0475: حرس NULL لتنبيهَي تأخر الطوابير — لا إنذار بلا دليل تشغيل
-- ---------------------------------------------------------------------
-- الجذر: في 0467 عومِل غياب السجل كتوقف: max(last_success) = NULL على
-- قاعدة حديثة الإنشاء كان يفسَّر '-infinity' فيفتح notification_dispatch_stalled
-- وintegration_outbox_lag زوراً بعد كل db reset (رُصد في CI: have:2 want:0).
-- الإصلاح: الشرطان يتطلبان قيمة غير فارغة أصلاً — لا صفوف pending في
-- الطابور = صحة، ولا تاريخ تشغيل ناجح بعد = تجاهل لا إنذار.
-- استبدال قياسي كامل؛ لا تغييرات أخرى عن 0473.
-- =====================================================================
