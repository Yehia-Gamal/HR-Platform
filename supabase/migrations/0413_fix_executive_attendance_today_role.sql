-- ============================================================================
-- 0413: إصلاح get_executive_attendance_today — تعريف المدير التنفيذي الصحيح
-- ============================================================================
-- المشكلة: الدالة كانت تعرّف المدير التنفيذي عبر employees.manager_id is null
-- لكن جدول employees لا يحتوي عمود manager_id إطلاقاً (العمود موجود فقط في
-- departments.manager_id و employee_departments.functional_manager_id).
-- → أي استدعاء للدالة يفشل فوراً بخطأ column does not exist.
--
-- الحل: تعريف المدير التنفيذي عبر الدور (executive-director / executive)
-- باستخدام public.current_has_active_role(text[]) — النموذج المعتمد في
-- الوصول والـ workspaces (0013/0053/0104).
-- باقي الدالة (المهام عبر work_assignment_participants، المواقع عبر
