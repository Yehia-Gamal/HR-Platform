-- 0092: Fix log_audit_event calls in archive_employee_secure and hard_delete_employee_guarded.
-- The 7th argument (p_description) expects text, but to_jsonb(v_employee) returns jsonb.
-- Fix: use row_to_json(v_employee)::text for the description, and move the full employee
-- data into the metadata jsonb.
...
begin
  ...
  perform public.log_audit_event('employee.archived','security','warning','employees',p_employee_id,trim(p_reason),row_to_json(v_employee)::text,jsonb_build_object('status','archived','sessionsRevoked',v_sessions,'devicesRevoked',v_devices,'userId',v_user_id));
...
begin
  ...
  perform public.log_audit_event('employee.permanent_delete_approved','security','critical','employees',p_employee_id,trim(p_reason),row_to_json(v_employee)::text,jsonb_build_object('confirmationCode',p_confirmation_code));
