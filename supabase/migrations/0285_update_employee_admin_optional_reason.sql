-- ============================================================================
-- 0285 — update_employee_admin: سبب التعديل اختياري
--   • p_reason أصبح له قيمة افتراضية (default null)
--   • عند غياب السبب يُسجَّل تلقائياً «تعديل عبر لوحة التحكم» في سجل التدقيق
--   (بناءً على أحدث نسخة من الدالة في 0191، مع تعديل معالجة السبب فقط)
-- ============================================================================
...
begin
  ...
  -- سبب التعديل اختياري — يُسجَّل تلقائياً عند الغياب
  v_reason := coalesce(nullif(trim(coalesce(p_reason, '')), ''), 'تعديل عبر لوحة التحكم');
  ...
  if p_changes ? 'jobTitleName' then
    ...
    if v_jt_name is not null then
      -- بحث عن مسمى وظيفي موجود (أو إنشاء جديد)
...
-- الصلاحيات — نفس التوقيع مع قيمة افتراضية، لا حاجة لإعادة REVOKE/GRANT
-- (التوقيع لم يتغير: uuid, jsonb, text — أُضيفت قيمة افتراضية فقط)
