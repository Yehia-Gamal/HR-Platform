-- 0069: Executive attendance today dashboard
-- Returns all active employees with today's attendance status (حضور، تأخر، غياب، مأمورية، إجازة).
-- Uses attendance_daily for the primary status + work_assignment_participants for external missions.
...
begin
  if not public.current_is_full_access() then
    raise exception 'full access required for attendance overview' using errcode = '42501';
  end if;
