-- 0520: السماح بتقديم الإجازات بأثر رجعي خلال الشهر الحالي
-- المشكلة: submit_my_request يمنع الإجازات الرجعية (annual, weekly_rest_comp, unpaid, mission, convoy, fundraising)
-- داخل الشهر الحالي ما لم يكن dayMark=true، ولكن dayMark يدعم يوم واحد فقط.
-- الحل: إزالةتقييد الشهر الحالي؛ فحص الشهر السابق (v_month_start) موجود بالفعل.
...
comment on function public.submit_my_request(text, text, text, jsonb, uuid) is
  '0520: تقديم الطلبات الذاتية مع السماح بالإجازات الرجعية خلال الشهر الحالي (بدون dayMark).';
