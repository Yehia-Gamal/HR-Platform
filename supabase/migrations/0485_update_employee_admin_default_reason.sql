-- =====================================================================
-- 0485: استعادة سلوك السبب الافتراضي لدى update_employee_admin
-- ---------------------------------------------------------------------
-- 0483_rebuild (إعادة البناء الكنونية لترميز العربية) أعاد تعريف
-- update_employee_admin محتفظاً بالـ default (p_reason text default '')
-- لكن مع رفع change_reason_required عند الفراغ — وهو ما يكسر اختبار
-- 0052#12 الذي يتوقع تحمّل السبب الفارغ وتسجيل السبب الافتراضي
-- 'تعديل بيانات الموظف من لوحة الإدارة'. هذه الـ migration تعيد بناء
-- البدن فقط (بلا تغيير في التوقيع) بنص عربي سليم ('غير مصرح') مع
-- التحمّل داخل البدن (السبب الفارغ/الأبيض → السبب الافتراضي).
-- =====================================================================
