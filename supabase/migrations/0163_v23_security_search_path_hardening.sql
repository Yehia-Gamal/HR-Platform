-- Migration 0163: Security — search_path hardening on utility functions (Agent 11)
...
COMMENT ON FUNCTION public.server_now() IS
  'Returns server UTC timestamp. Pinned search_path (0163 hardening).';
...
COMMENT ON FUNCTION public.set_updated_at() IS
  'Trigger helper: sets updated_at = now() on row UPDATE. Pinned search_path (0163 hardening).';
...
COMMENT ON FUNCTION public.kpi_effective_deadline(public.kpi_cycles) IS
  'Returns effective KPI deadline (extended or original). Pinned search_path (0163 hardening).';
