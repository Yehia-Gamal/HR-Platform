-- =====================================================================
-- 0127: كشف الحضور والانصراف الشهري لكل موظف (V12 §18)
-- =====================================================================
-- الهدف: RPC يُرجع كشفًا شهريًا مفصّلًا يتضمن:
--   * بيانات الموظف (الاسم، الكود، الإدارة، المدير، الفرع).
--   * جدول يومي (التاريخ، اليوم، الحضور، الانصراف، الوردية، الساعات الفعلية،
--     المطلوبة، التأخير، الخروج المبكر، الإضافي، الإجازة، الإذن، المأمورية،
--     القافلة/الفاندي، نسيان ختم، غياب، حالة اليوم، ملاحظات، تصحيح).
--   * ملخصات (أيام الحضور/الغياب/الإجازات/المأموريات/التأخير/...).
-- المصادر: attendance_daily, attendance_events, leave_requests, work_assignments,
--   attendance_corrections, attendance_permits, public_holidays, roster_days, shifts.
-- الحساب Server-side بتوقيت Africa/Cairo. timestamps مخزنة UTC.
-- =====================================================================

-- ---------------------------------------------------------------------
-- 1) دالة مشتركة تبني الكشف لموظف + شهر.
--    تُستدعى من RPC العام (بصلاحية) ومن RPC الشخصي (ذاتي).
-- ---------------------------------------------------------------------
...
begin
  ...
  while v_day <= v_end loop
    ...
    -- الوردية (متغيرات قياسية تُصفّر كل دورة لتفادي record غير مُسند)
    ...
    -- هل هو يوم راحة (الجمعة/السبت أو من الروستر)؟
    elsif extract(isodow from v_day) in (5, 6) then
    ...
    -- هل هو روستر راحة/عطلة؟
      ...
      else v_holiday_days := v_holiday_days + 1; end if;
    else
      -- يوم مجدول — نحسبه
      ...
      -- حاضر/متأخر/جزئي/غائب
        ...
        if v_row.status = 'absent' then v_absent_days := v_absent_days + 1;
        elsif v_row.status in ('present','late','partial') then v_present_days := v_present_days + 1;
        end if;
      else
    ...
    -- إذن حضور؟
    if exists (select 1 from public.attendance_permits p
               where p.employee_id = p_employee_id and p.permit_date = v_day
                 and p.status = 'approved') then
    ...
    v_day_obj := jsonb_build_object(
      ...
      'hasPermit', exists(select 1 from public.attendance_permits p
                          where p.employee_id = p_employee_id and p.permit_date = v_day and p.status = 'approved'),
...
comment on function public._build_attendance_statement(uuid, integer, integer) is
  'V12 §18: دالة مشتركة تبني كشف الحضور والانصراف الشهري لموظف. تُستدعى من RPCs العامة والشخصية.';

-- لا تُمنح مباشرة — تُستدعى من RPCs أدناه فقط.
revoke execute on function public._build_attendance_statement(uuid, integer, integer) from public, authenticated;

-- ---------------------------------------------------------------------
-- 2) RPC الشخصي: الموظف يرى كشفه فقط.
-- ---------------------------------------------------------------------
create or replace function public.get_my_monthly_attendance_statement(
...
begin
  if v_me is null then raise exception 'NO_EMPLOYEE_LINKED' using errcode = '42501'; end if;
  if p_month < 1 or p_month > 12 then raise exception 'INVALID_MONTH' using errcode = '22023'; end if;
  return public._build_attendance_statement(v_me, p_year, p_month);
end $$;

comment on function public.get_my_monthly_attendance_statement(integer, integer) is
  'V12 §18: كشف الحضور والانصراف الشهري للموظف الحالي (ذاتي).';

revoke execute on function public.get_my_monthly_attendance_statement(integer, integer) from public;
grant execute on function public.get_my_monthly_attendance_statement(integer, integer) to authenticated;

-- ---------------------------------------------------------------------
-- 3) RPC العام: المدير/HR/Main Admin يرى كشف أي موظف وفق الصلاحية.
-- ---------------------------------------------------------------------
...
begin
  if p_month < 1 or p_month > 12 then raise exception 'INVALID_MONTH' using errcode = '22023'; end if;
  -- الصلاحية: full-access أو قراءة الحضور أو الوصول للموظف (مدير مباشر).
  if not (
    public.current_is_full_access()
    or public.has_any_permission(array['attendance.record.read','reports.attendance.read'])
...
comment on function public.get_employee_monthly_attendance_statement(uuid, integer, integer) is
  'V12 §18: كشف الحضور والانصراف الشهري لموظف محدد (المدير/HR/Main Admin).';

revoke execute on function public.get_employee_monthly_attendance_statement(uuid, integer, integer) from public;
grant execute on function public.get_employee_monthly_attendance_statement(uuid, integer, integer) to authenticated;

-- =====================================================================
-- نهاية Migration 0127
-- =====================================================================
