...
-- Migration 0440: إصلاح رفض اعتماد الطلبات القديمة المتجاوزة لمهلة التصعيد (403).
--
-- المشكلة (إنتاج): طلبات قديمة تجاوزت مهلة المدير المباشر (أيام) رُفض قرارها
--   بـ 403 على decide_request رغم أن الفاعل هو مدير التشغيل 1 (أبو عمار) وهو
--   الوجهة الرسمية للتصعيد. السبب: الصلاحية تشترط v_current_step >= 2 بينما:
--   (أ) طلبات بقي خطوتها 1 active والمهلة منتهية (الكروان لم يصعّدها لغياب خطوة تالية
--       أو لخلل في معالجة دفعة) → v_current_step = 1 → رفض.
--   (ب) طلبات بلا خطوة 2 أصلاً (أقدم البنى) ووسمُها escalated → رفض.
-- الحل: يمنح مدير التشغيل 1 صلاحية القرار أيضاً عندما تتجاوز مهلة الخطوة/الطلب
--   (escalation_deadline) الحالية، أو عندما يكون الطلب في حالة تصعيد فعلية
--   ('escalated' / 'awaiting_operator') — مع بقاء قيد can_access_employee
--   وحماية الطلبات الجديدة (المدة لم تنتهِ بعد).
...
begin
  ...
  --   الخطوة/الطلب متجاوزة (طلبات قديمة عالقة — 0440)، أو في حالة تصعيد فعلية.
  if not found then
    v_authorized :=
...
comment on function public.decide_request(uuid, text, text) is
  '0406: قرار — مدير مباشر(أي وقت) / أبو عمار operations-manager-1 (step>=2 أو مهلة متجاوزة — 0440) / full_access. بدون HR. موافقة واحدة تُنهي الطلب.';
