-- 0402: إصلاح انحدار ترتيب فحص الجمعة (weekend) في لوحة الحضور التنفيذية
--
-- نية 0333 (والاختبار 0116): فحص isodow=5 (الجمعة/weekend) يجب أن يأتي
-- fallback بعد on_leave و assignment — فالموظف في إجازة أو مهمة يوم الجمعة
-- يُعرض كـ on_leave/assignment لا كـ weekend.
--
-- migration 0391 (mv_executive_attendance_snapshot) أعاد كتابة الدالة والـ MV
-- وجعل فحص weekend أول حالة في CASE، فانكسر السلوك: الجمعة + إجازة/مهمة =
-- weekend. هذا الـ migration يعيد الترتيب الصحيح في الاثنين معاً.

begin;

-- ════════════════════════════════════════════════════════
-- 1) إصلاح ترتيب CASE في get_executive_attendance_overview
-- ════════════════════════════════════════════════════════
...
-- ════════════════════════════════════════════════════════
-- 2) إصلاح ترتيب CASE في الـ MV (لقراءة اليوم الحالي)
-- ════════════════════════════════════════════════════════

drop materialized view if exists public.mv_executive_attendance_snapshot;

create materialized view public.mv_executive_attendance_snapshot as
