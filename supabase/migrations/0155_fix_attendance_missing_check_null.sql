-- =====================================================================
-- 0155: إصلاح NULL في missingCheckIn / missingCheckOut بكشف الحضور
-- =====================================================================
-- المشكلة: عندما لا يوجد صف attendance_daily لليوم (v_row.status = NULL)،
-- تعبير NOT IN يُرجع NULL (منطق ثلاثي القيم في PostgreSQL)، مما يجعل
-- سلسلة AND تُقيّم إلى NULL بدل false. النتيجة: Zod يرفض القيمة لأنه
-- يتوقع boolean وليس null.
-- الحل: لف التعبيرات في coalesce(..., false).
-- =====================================================================
...
begin
  ...
  while v_day <= v_end loop
    ...
    v_day_obj := jsonb_build_object(
      ...
      -- 0155-fix: coalesce لمنع NULL من المنطق الثلاثي في PostgreSQL
      'missingCheckIn', coalesce(
        v_row.first_check_in is null and v_row.status not in ('absent','on_leave','holiday','weekend')
          and v_status not like '%عطلة%' and v_status not like '%راحة%'
          and v_status <> 'إجازة معتمدة' and v_status not like '%مأمورية%'
          and v_status not like '%قافلة%' and v_status not like '%فاندي%',
        false),
      'missingCheckOut', coalesce(
        v_row.last_check_out is null and v_row.first_check_in is not null,
        false),
...
comment on function public._build_attendance_statement(uuid, integer, integer) is
  'V12 §18: دالة مشتركة تبني كشف الحضور والانصراف الشهري لموظف. تُستدعى من RPCs العامة والشخصية. (0155: إصلاح NULL في missingCheckIn/Out)';

-- صلاحيات بدون تغيير — تُستدعى من RPCs أدناه فقط.
revoke execute on function public._build_attendance_statement(uuid, integer, integer) from public, authenticated;

-- =====================================================================
-- نهاية Migration 0155
-- =====================================================================
