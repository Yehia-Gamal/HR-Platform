-- 0083: Preserve independent live-location requests and atomically activate
-- server-verified passkeys in both canonical credential/device registries.
...
begin
  ...
  if not exists (
    ...
    where id = p_employee_id
      and status in ('active', 'invited', 'onboarding')
      and user_id is not null
  ) then
    raise exception 'employee is not active or has no linked user account' using errcode = 'P0002';
  end if;

  -- Each send is an independent request. Only rate-limit creation; never mutate
  -- an earlier pending/accepted/active request as a side effect of resending.
  if exists (
  ...
  insert into public.live_location_requests(
    ...
    'pending', 'verification', now(), now() + interval '5 minutes',
    v_duration,
    jsonb_build_object(
      'mode', p_mode,
      'videoSeconds', v_video_seconds,
      'needsPoint', p_mode <> 'video_5s',
      'needsVideo', v_needs_video
    ),
    auth.uid()
  ) returning * into v_row;

  -- Keep the canonical row id in metadata for consistent FCM/client contracts.
  ...
  if v_target_user is not null then
    insert into public.notifications(
      ...
      category, priority, action_url, entity_type, entity_id, metadata, created_by
    ) values (
  ...
  perform public.log_audit_event(
    'live_location_requested', 'security', 'warning',
    'live_location_requests', v_row.id, 'طلب موقع حي', null,
    jsonb_build_object('mode', p_mode, 'employeeId', p_employee_id, 'requestId', v_row.id)
  );
...
begin
  ...
  insert into public.employee_devices(
    ...
    left(coalesce(p_device_label, 'هاتف الموظف'), 120), 'android', 'active', now(),
    jsonb_build_object(
      'serverVerified', true,
