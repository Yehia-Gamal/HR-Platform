-- migration: 0434
-- description: إصلاح تصنيف يوم «فاندي» في الكشف الشهري — v287 كانت تُظهره «قافلة»
-- -----------------------------------------------------------------------------
-- السبب الجذري:
--   * _build_attendance_statement_v287 (من 0430) كان يقرر الحالة من أعلام الطبقات
--     السفلية: hasMission → «مأمورية»، hasConvoyFundi → «قافلة».
--   * hasConvoyFundi=true لليومين «قافلة» و«فاندي» معاً (تحدده الطبقة v186 عبر
--     LIKE '%قافلة%' OR LIKE '%فاندي%')، فكان يوم الفاندي يُعرض «قافلة» دائماً
--     قبل الوصول إلى فرع v_type='fundraising' → «فاندي».
-- الإصلاح: التحقق من التصنيف الداخلي أولاً — إن كان اليوم «فاندي» فيبقى «فاندي»
--          (الطبقات السفلية v186/v266 تصنّف بدقة من payload / work_assignments)،
--          وإلا تُعيد v287 «قافلة» عن يوم القافلة.
