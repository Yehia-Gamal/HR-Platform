-- ============================================================================
-- 0508_fix_leave_requests_admin_security_definer.sql
--
-- ترقية دالة get_leave_requests_admin إلى SECURITY DEFINER وتوسيع الصلاحيات:
-- 1) منع فشل الاستعلام بسبب تداخل RLS عند فحص مجموع إجازات الموظفين إدارياً.
-- 2) توسيع شرط الصلاحية ليشمل الأدوار القيادية (admin, hr-manager, executive, etc.)
--    وصلاحيات approve و policy.manage.
-- ============================================================================

create or replace function public.get_leave_requests_admin(
  p_year integer default null::integer,
  p_status text default null::text,
  p_leave_type text default null::text,
...
comment on function public.get_leave_requests_admin(integer, text, text, uuid, integer, integer) is
  '0508: جلب طلبات الإجازات للأدوار الإدارية بصلاحية SECURITY DEFINER وتوسيع الصلاحيات.';
revoke all on function public.get_leave_requests_admin(integer, text, text, uuid, integer, integer) from public, anon;
