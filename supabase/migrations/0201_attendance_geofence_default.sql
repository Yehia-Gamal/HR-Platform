-- ============================================================================
-- 0201: إضافة السياج الجغرافي الافتراضي + fallback في دوال الحضور
-- ============================================================================
-- المشكلة: الموظفون بلا roster_days.geofence_id ولا shift_assignments.geofence_id
--   يحصلون على خطأ "attendance_geofence_not_configured" عند تسجيل البصمة.
-- الحل:
--   1) إنشاء سياج جغرافي لمجمع أحلى شباب (منيل شيحة) — نقطة واحدة لجميع الموظفين.
--   2) تحديث shift_assignments الحالية بدون geofence_id لتشير إلى السياج الجديد.
--   3) إضافة fallback في record_attendance_event و record_attendance_local_biometric:
--      إذا لم يُعثر على سياج عبر الجدول الزمني أو تعيين الوردية، يُؤخذ أول سياج نشط.
--      هذا يضمن عمل البصمة لموظفين جدد بدون تعيين وردية.
-- ============================================================================

begin;

-- ═══════════════════════════════════════════════════════════════════════════════
-- القسم أ: بذرة السياج الجغرافي
-- ═══════════════════════════════════════════════════════════════════════════════

insert into public.geofences (code, name, latitude, longitude, radius_meters, max_accuracy, is_active)
values (
  'AHLA_SHABAB_MAIN',
...
-- ═══════════════════════════════════════════════════════════════════════════════
-- القسم ب: ربط shift_assignments الحالية بالسياج الافتراضي
-- ═══════════════════════════════════════════════════════════════════════════════

update public.shift_assignments
set geofence_id = (select id from public.geofences where code = 'AHLA_SHABAB_MAIN'),
    updated_at = now()
where geofence_id is null
  and is_active = true;

-- ═══════════════════════════════════════════════════════════════════════════════
-- القسم ج: record_attendance_event — إضافة fallback للسياج الجغرافي
-- ═══════════════════════════════════════════════════════════════════════════════
-- نسخة مطابقة لـ 0169 مع تغيير واحد فقط في الخطوة 11:
--   إذا لم يُعثر على سياج عبر roster أو assignment، يُؤخذ أول سياج نشط.
...
-- ═══════════════════════════════════════════════════════════════════════════════
-- القسم د: record_attendance_local_biometric — نفس الـ fallback
-- ═══════════════════════════════════════════════════════════════════════════════
