-- Migration 0153: fix full_name → full_name_ar in KPI notification functions.
-- ============================================================================
-- خلفية: Migration 0146 تستخدم `full_name` في advance_kpi_stage و return_kpi_stage
-- لكن جدول employees يحوي `full_name_ar` و `full_name_en` فقط — لا يوجد عمود `full_name`.
-- هذا يسبب خطأ runtime عند محاولة إرسال إشعار بعد انتقال مرحلة KPI.
-- ============================================================================

-- ── 1) إصلاح advance_kpi_stage ──
...
begin
 ...
 select full_name_ar into v_emp_name from public.employees where id=v_eval.employee_id;
...
comment on function public.advance_kpi_stage(uuid,text,jsonb,text) is
  'V17 §10+§9.2.2: تقدّم مرحلة تقييم KPI مع إشعار الطرف التالي. 0153: إصلاح full_name → full_name_ar.';
comment on function public.return_kpi_stage(uuid,text,text) is
  'V17 §10+§9.2.2: إعادة مرحلة تقييم KPI مع إشعار الطرف المُعاد إليه. 0153: إصلاح full_name → full_name_ar.';
