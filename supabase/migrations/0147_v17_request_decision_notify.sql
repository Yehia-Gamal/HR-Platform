-- 0145: V17 §9.2.3 — إشعار الموظف عند اتخاذ قرار على طلبه.
-- يضيف استدعاء notify_employee داخل decide_request عند:
--   • الموافقة النهائية (approved)
--   • الرفض (rejected)
--   • الإعادة (returned)
-- لا يُشعر عند الانتقال لخطوة وسيطة (in_review).
-- ============================================================================
...
begin
  ...
  if not found then
    ...
    -- §9.2.3: إشعار الموظف بالقرار (مسار احتياطي)
  ...
  if p_decision in ('reject', 'return') then
    ...
    -- §9.2.3: إشعار الموظف بالرفض أو الإعادة
    perform public.notify_employee(
      v_req.employee_id,
      case v_req.status
        when 'rejected' then 'تم رفض طلبك'
  ...
  if found then
    ...
    -- لا إشعار هنا — الطلب لم يُحسم بعد (انتقل لخطوة وسيطة)
  else
    update public.workflow_instances
      set status = 'completed', completed_at = now(), updated_at = now()
    where request_id = p_request_id and status = 'running';
    ...
    -- §9.2.3: إشعار الموظف بالموافقة النهائية
    perform public.notify_employee(
      v_req.employee_id,
      ...
      'request',
      v_req.id,
      jsonb_build_object('decision', 'approve', 'request_type', v_req.request_type)
    );
  end if;
...
comment on function public.decide_request(uuid, text, text) is
  'V17 §9.2.3: قرار على الطلب (approve/reject/return) مع إشعار الموظف. 0145: أُضيف notify_employee عند كل قرار نهائي.';
