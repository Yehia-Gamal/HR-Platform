-- =====================================================================
-- 0301_fix_kpi_admin_grants_and_diagnostics.sql
-- =====================================================================
-- الجذر المؤكد لعطلَي واجهة «تجهيز دورة KPI» (9A15C484 / 930B96FA / 0DBF274F):
--
-- 0214 أعاد تعريف create_kpi_cycle_admin إلى توقيع 8-معاملات (بإضافة
-- p_use_parallel_flow)، لكن GRANT الوحيد المنشور استهدف التوقيع القديم
-- 7-معاملات من 0058. التوقيع الثماني الحالي لا يملك EXECUTE لـ authenticated
-- على الإطلاق → أي ضغطة «تجهيز الدورة» تُسقط بـ 42501 permission denied،
-- ويُغطّيه errorMapper بفقاعة «حدث خطأ غير متوقع».
--
-- ما يفعله هذا الـ migration:
--   (1) إسقاط overload قديم 7-معاملات (إن بقي) من create_kpi_cycle_admin.
--   (2) REVOKE/GRANT صريحة ومتسقة على التوقيعات الحالية لكل دوال KPI.
--   (3) NOTIFY pgrst 'reload schema' لإجبار PostgREST على تحديث الذاكرة.
--   (4) دالة تشخيص kpi_diag_run(p_month) تعيد jsonb شامل.
--   (5) دالة create_kpi_cycle_admin_safe محصّنة تعيد سبب الفشل بالعربية.
--
-- Idempotent. لا يغيّر أي منطق إنتاجي موجود.
-- =====================================================================

begin;

-- ---------------------------------------------------------------------
-- 1) إزالة overload قديم 7-معاملات لـ create_kpi_cycle_admin
-- ---------------------------------------------------------------------
drop function if exists public.create_kpi_cycle_admin(date, uuid, timestamptz, timestamptz, timestamptz, timestamptz, boolean);

-- ---------------------------------------------------------------------
-- 2) منح authenticated على التوقيعات الحقيقية
-- ---------------------------------------------------------------------
...
-- refresh_kpi_attendance_inputs (0065 — أحدث تعريف)
...
-- send_kpi_notifications_admin (0206)
...
-- create_kpi_policy_version — منح ديناميكي حسب التوقيع الحالي
do $$
declare
  v_sig text;
begin
...
-- ---------------------------------------------------------------------
-- 3) إجبار PostgREST على إعادة تحميل schema cache فورًا
-- ---------------------------------------------------------------------
...
comment on function public.kpi_diag_run(date) is
  'فحص شامل لأسباب فشل KPI cycles على التطبيق. يعيد jsonb بتقرير كامل عن الدوال والجداول والأعمدة والمنح واستدعاء حقيقي محمي بـROLLBACK. منشأ بواسطة 0301.';
...
comment on function public.create_kpi_cycle_admin_safe(date, uuid, timestamptz, timestamptz, timestamptz, timestamptz, boolean, boolean) is
  'نسخة محمية من create_kpi_cycle_admin تعيد سبب الفشل بالعربية بدل رفع خطأ غير معروف. منشأة بواسطة 0301.';
