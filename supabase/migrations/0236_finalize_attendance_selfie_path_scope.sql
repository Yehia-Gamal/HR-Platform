-- 0236_finalize_attendance_selfie_path_scope.sql
-- ═══════════════════════════════════════════════════════════════════════
-- تقييد p_selfie_path داخل finalize_verified_attendance بمجلّد الموظف نفسه.
--
-- الخلفية الأمنية: محفّز 0235 (is_safe_storage_path) يرفض المخططات و ../
-- والمسار المطلق ومحارف التحكّم — لكنه محفّز على مستوى الصفّ فلا يرى هوية
-- الموظف الذي يسجّل الحضور، لذا لا يستطيع فرض «الملكية»: القيمة
-- «<موظف_آخر>/2026/secret.jpg» تمرّ عبر 0235 رغم أنها خارج مجلّد صاحب البصمة.
--
-- هذه الدالة SECURITY DEFINER تملك p_employee_id في نطاقها، فهي الطبقة
-- الوحيدة القادرة على تثبيت المسار على شكل:  <p_employee_id>/<yyyy>/<اسم_الملف>
-- ومنع الوصول/الكتابة إلى مجلّد موظف آخر (مطابقة لنمط register_live_location_video).
--
-- التغيير الوحيد عن 0215: إضافة كتلة التحقّق من p_selfie_path بعد فحص
-- CHECK_IN/CHECK_OUT وقبل INSERT الخاص بالـ idempotency. باقي المنطق حرفياً كما في 0215.
-- ═══════════════════════════════════════════════════════════════════════
...
begin
  ...
  if p_selfie_path is not null then
    ...
    if position(chr(0) in p_selfie_path) > 0 then
      raise exception 'invalid_selfie_path_bad_bytes' using errcode = '22023';
    end if;
...
comment on function public.finalize_verified_attendance is
  '0236: 0215 + employee-scoped selfie_path validation (<employee_id>/<yyyy>/<file>, no traversal/absolute/scheme/backslash/cross-employee).';
