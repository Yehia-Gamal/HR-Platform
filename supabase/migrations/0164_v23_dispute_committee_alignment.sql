-- 0164_v23_dispute_committee_alignment.sql
-- V23 Agent 07: Dispute committee alignment
-- 1. Add resolved_friendly status to CHECK constraint
-- 2. Update transition_dispute_case: resolve_friendly action + close from resolved_friendly
-- 3. Create submit_my_dispute_v23 simplified 7-param RPC
-- 4. Update get_committee_dispute_portal summary with resolvedFriendly count

-- ═══════════════════════════════════════════════════════════════════════════════
-- 1. Expand status CHECK to include resolved_friendly
-- ═══════════════════════════════════════════════════════════════════════════════
...
begin
 ...
 if p_action='close' then
  -- V23: also allow close from resolved_friendly
  if v.status not in ('decision_issued','settlement_pending','resolved_friendly') then raise exception 'CLOSE_NOT_ALLOWED'; end if;
 ...
 -- V23: add resolved_friendly to force_status allowed list
...
begin
  -- Delegate to the full submit_my_dispute with safe defaults
  return public.submit_my_dispute(
    ...
    'normal',           -- priority
    null,               -- incident_at
    null,               -- incident_location
    p_parties,
    p_witnesses,
    null,               -- direct_manager_contacted
    null,               -- amicable_attempted
    null,               -- amicable_result
    null,               -- requested_action
    true,               -- confidential (default true)
    p_truth_confirmed,
    p_confidentiality_accepted
  );
end $$;
