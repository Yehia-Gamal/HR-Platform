...
-- without seeding challenge/credential/device rows. A validly-scoped path passes
-- the selfie gate and then fails on the attempt-row FK (23503), which is proof
...
-- ⑨ A validly-scoped path passes the selfie check → proceeds to the idempotency
-- INSERT, which fails on the FK (dummy ids) with 23503. Reaching 23503 proves
-- the selfie gate was passed.
