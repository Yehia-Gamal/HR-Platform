...
select ok(exists(select 1 from pg_policies where schemaname='storage' and tablename='objects' and policyname='employee_avatars_public_read'),'public avatar read policy remains installed');
...
select is((select count(*)::integer from storage.objects where bucket_id='employee-avatars' and name='admin/managed.webp'),1,'managed avatar insert is visible inside the transaction');
